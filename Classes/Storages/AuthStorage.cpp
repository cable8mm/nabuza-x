...
// 2. 저장되어 있다면 사용. 사용 시 토큰 에러가 발생하면 각 API 마다 예외가
// 처리해 줌.
...
const char *AUTH_DEFAULT_STORAGE_ACCESS_TOKEN_KEY =
...
void AuthStorage::onReceivingAccessToken() {
    ...
    oss << "consumer_key=" << CONSUMER_KEY << "&signature=" << SIGNATURE
        << "&appid=" << this->getAppId()->getCString();

    AnytaleHTTP::onHttpRequestingWithoutToken(
        this, callfuncND_selector(AuthStorage::onHttpRequestCompleted),
        "/access_tokens/get", oss.str().c_str(), "HTTP_GET_ACCESS_TOKEN");
}

void AuthStorage::onHttpRequestCompleted(CCHttpClient *sender,
    ...
    // Access Token 얻기
    if (strcmp(response->getHttpRequest()->getTag(), "HTTP_GET_ACCESS_TOKEN") ==
        0) {
        /*
...
void AuthStorage::setAccessToken() {
    this->accessToken = CCUserDefault::sharedUserDefault()->getStringForKey(
        AUTH_DEFAULT_STORAGE_ACCESS_TOKEN_KEY, "");
...
void AuthStorage::setAccessToken(std::string accessToken) {
    CCUserDefault::sharedUserDefault()->setStringForKey(
        AUTH_DEFAULT_STORAGE_ACCESS_TOKEN_KEY, accessToken);
...
void AuthStorage::setAppId() {
    this->appId = CCUserDefault::sharedUserDefault()->getStringForKey(
        AUTH_DEFAULT_STORAGE_APP_ID_KEY);

    if (this->appId == "") {
        this->appId = GenerateAppId();
        CCUserDefault::sharedUserDefault()->setStringForKey(
            AUTH_DEFAULT_STORAGE_APP_ID_KEY, this->appId);
    }
}
