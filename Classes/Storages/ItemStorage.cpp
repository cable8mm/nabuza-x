...
void ItemStorage::addItem(int itemKey) {
    // itemKey는 0부터 6까지 있다. 이 것들의 배열을 스트링으로 넣기 위해서 1을
    // 더한다.
    int itemStorages =
        this->userDefault->getIntegerForKey(USER_DEFAULT_ITEMS_KEY, 0);
...
CCArray* ItemStorage::getItems() {
    ...
    unsigned int itemStorages =
        this->userDefault->getIntegerForKey(USER_DEFAULT_ITEMS_KEY);
