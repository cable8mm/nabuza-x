...
const char *USER_DEFAULT_USER_STORAGE_CARD_COUNT_KEY =
    "USER_STORAGE_CARD_COUNT_KEY";
const char *USER_DEFAULT_USER_STORAGE_CARD_LAST_UPDATED_KEY =
    "USER_STORAGE_CARD_LAST_UPDATED_KEY";
const char *USER_DEFAULT_USER_STORAGE_IS_FIRST_KEY =
    "USER_STORAGE_IS_FIRST_KEY";
const char *USER_DEFAULT_USER_STORAGE_NICKNAME_KEY =
    "USER_STORAGE_NICKNAME_KEY";
const char *USER_DEFAULT_USER_STORAGE_OWN_JADE_COUNT_KEY =
    "USER_STORAGE_OWN_JADE_COUNT_KEY";
const char *USER_DEFAULT_USER_STORAGE_OWN_GOLD_KEY =
    "USER_STORAGE_OWN_GOLD_KEY";
const char *USER_DEFAULT_USER_STORAGE_INVITATION_COUNT_KEY =
    "USER_STORAGE_INVITATION_COUNT_KEY";
const char *USER_DEFAULT_USER_STORAGE_LAST_LEVEL_KEY =
    "USER_STORAGE_LAST_LEVEL_KEY";
const char *USER_DEFAULT_USER_STORAGE_HIGHSCORE_KEY =
    "USER_STORAGE_HIGHSCORE_KEY";
const char *USER_DEFAULT_USER_STORAGE_COUNTDOWN_CLOCK =
    "USER_STORAGE_COUNTDOWN_CLOCK";
const char *USER_DEFAULT_USER_STORAGE_SETTING_IS_BGM =
    "USER_STORAGE_SETTING_IS_BGM";
const char *USER_DEFAULT_USER_STORAGE_SETTING_IS_EFFECT =
    "USER_STORAGE_SETTING_IS_EFFECT";
const char *USER_DEFAULT_USER_STORAGE_SETTING_IS_VIBRATE =
    "USER_STORAGE_SETTING_IS_VIBRATE";
const char *USER_DEFAULT_USER_STORAGE_SETTING_IS_EVENTALARM =
    "USER_STORAGE_SETTING_IS_EVENTALARM";
...
bool UserStorage::init() {
    this->reset();
    if (CCUserDefault::sharedUserDefault()->getBoolForKey(
            USER_DEFAULT_USER_STORAGE_IS_FIRST_KEY, false)) {
        this->isFirst = true;
    } else {
        this->isFirst = false;
    }

    this->cardCount = CCUserDefault::sharedUserDefault()->getIntegerForKey(
        USER_DEFAULT_USER_STORAGE_CARD_COUNT_KEY, 5);  // default 5
...
void UserStorage::reset() {
    CCUserDefault::sharedUserDefault()->setBoolForKey(
        USER_DEFAULT_USER_STORAGE_IS_FIRST_KEY, false);
    this->ownJadeCount = CCUserDefault::sharedUserDefault()->getIntegerForKey(
        USER_DEFAULT_USER_STORAGE_OWN_JADE_COUNT_KEY, 7);  // default 7
    this->ownGold = CCUserDefault::sharedUserDefault()->getIntegerForKey(
        USER_DEFAULT_USER_STORAGE_OWN_GOLD_KEY, 7000);  // default 7000

    this->lastLevel = CCUserDefault::sharedUserDefault()->getIntegerForKey(
        USER_DEFAULT_USER_STORAGE_LAST_LEVEL_KEY, 0);  // default 7
    this->highscore = CCUserDefault::sharedUserDefault()->getIntegerForKey(
        USER_DEFAULT_USER_STORAGE_HIGHSCORE_KEY, 0);  // default 7

    this->isBgm = CCUserDefault::sharedUserDefault()->getBoolForKey(
        USER_DEFAULT_USER_STORAGE_SETTING_IS_BGM, true);
    this->isEffect = CCUserDefault::sharedUserDefault()->getBoolForKey(
        USER_DEFAULT_USER_STORAGE_SETTING_IS_EFFECT, true);
    this->isVibrate = CCUserDefault::sharedUserDefault()->getBoolForKey(
        USER_DEFAULT_USER_STORAGE_SETTING_IS_VIBRATE, true);
    this->isEventalarm = CCUserDefault::sharedUserDefault()->getBoolForKey(
        USER_DEFAULT_USER_STORAGE_SETTING_IS_EVENTALARM, true);
}

int UserStorage::getCardCount() {
    CCAssert(this->lastCardCountUpdated != 0,
             "LastCardCountUpdated must not ZERO");

    if (this->cardCount < 5) {
        unsigned int diffSeconds = this->getDiffSeconds();
        int updatingCardCount =
            (int)floorf(diffSeconds / REMAIN_SECONDS_FOR_CARD);
        CCLog("diffSeconds : %u, UpdatingCardCount : %d", diffSeconds,
              updatingCardCount);
        //        CCAssert(updatingCardCount > -1 && updatingCardCount < 6,
        //        "UpdatingCardCount must between 0 - 5");
...
void UserStorage::setCardCount(int v) {
    ...
    CCUserDefault::sharedUserDefault()->setIntegerForKey(
        USER_DEFAULT_USER_STORAGE_CARD_COUNT_KEY, this->cardCount);
    ...
    CCNotificationCenter::sharedNotificationCenter()->postNotification(
        "OWN_CARD_COUNT_CHANGED_NOTIFICATION", ownCardCount);
}
...
void UserStorage::setLastCardCountUpdated(unsigned int v) {
    ...
    CCUserDefault::sharedUserDefault()->setIntegerForKey(
        USER_DEFAULT_USER_STORAGE_CARD_LAST_UPDATED_KEY, v);
}
...
void UserStorage::addCard(int cardCount) {
    ...
    CCLog("tmpCardCount / this->cardCount / cardCount : %d, %d, %d",
          tmpCardCount, this->cardCount, cardCount);
...
void UserStorage::setNickname(std::string v) {
    CCUserDefault::sharedUserDefault()->setStringForKey(
        USER_DEFAULT_USER_STORAGE_NICKNAME_KEY, v);
...
void UserStorage::setLastLevel(int v) {
    CCUserDefault::sharedUserDefault()->setIntegerForKey(
        USER_DEFAULT_USER_STORAGE_LAST_LEVEL_KEY, v);
...
void UserStorage::setHighscore(int v) {
    CCUserDefault::sharedUserDefault()->setIntegerForKey(
        USER_DEFAULT_USER_STORAGE_HIGHSCORE_KEY, v);
    this->highscore = v;
}
...
void UserStorage::setCountDownClock() {
    CCUserDefault::sharedUserDefault()->setIntegerForKey(
        USER_DEFAULT_USER_STORAGE_COUNTDOWN_CLOCK, GetClockCount());
}

int UserStorage::getCountDownClock() {
    return CCUserDefault::sharedUserDefault()->getIntegerForKey(
        USER_DEFAULT_USER_STORAGE_COUNTDOWN_CLOCK, 0);
}
...
void UserStorage::setIsBgm(bool v) {
    ...
    this->isBgm = v;
    CCUserDefault::sharedUserDefault()->setBoolForKey(
        USER_DEFAULT_USER_STORAGE_SETTING_IS_BGM, v);
}
...
void UserStorage::setIsEffect(bool v) {
    ...
    this->isEffect = v;
    CCUserDefault::sharedUserDefault()->setBoolForKey(
        USER_DEFAULT_USER_STORAGE_SETTING_IS_EFFECT, v);
}
...
void UserStorage::setIsVibrate(bool v) {
    ...
    this->isVibrate = v;
    CCUserDefault::sharedUserDefault()->setBoolForKey(
        USER_DEFAULT_USER_STORAGE_SETTING_IS_VIBRATE, v);
}
...
void UserStorage::setisEventalarm(bool v) {
    ...
    this->isEventalarm = v;
    CCUserDefault::sharedUserDefault()->setBoolForKey(
        USER_DEFAULT_USER_STORAGE_SETTING_IS_EVENTALARM, v);
}
...
void LevelInformation::setting(unsigned int ownGoldCount) {
    ...
    for (unsigned int i = 0;
         i < sizeof(basicGoldCounts) / sizeof(basicGoldCounts[0]) - 1; i++) {
        if (basicGoldCounts[i] < ownGoldCount &&
            ownGoldCount <= basicGoldCounts[i + 1]) {
            this->needGoldCountForNextLevel =
                basicGoldCounts[i + 2] - ownGoldCount;
            this->setKey(i + 1);
            return;
        }
    }
...
void LevelInformation::setKey(unsigned int key) {
    ...
    CCLog("levelBonus : %d, maxNeedGoldCount : %d, needGoldCOuntForNextLevel : "
          "%d",
          this->levelBonus, this->maxNeedGoldCount,
          this->needGoldCountForNextLevel);
}
