...
LevelIconSprite* LevelIconSprite::createWithItemKey(unsigned int key) {
    ...
    if (pobSprite && pobSprite->initWithFile("level_icon.png", CCRect(key * TEXTURE_SIZE_W, 0., TEXTURE_SIZE_W, TEXTURE_SIZE_H)))
    {
...
LevelIconSprite* LevelIconSprite::createWithGoldCount(unsigned int goldCount) {
    ...
    if (goldCount == 0) {
        ...
        for (unsigned int i=0; i< sizeof(basicGoldCounts)/sizeof(basicGoldCounts[0]) - 1;i++) {
            if (basicGoldCounts[i] < goldCount && goldCount <= basicGoldCounts[i+1]) {
                level = i+1;
                break;
            }
        }
    ...
    if (pobSprite && pobSprite->initWithFile("level_icon.png", CCRect(level * TEXTURE_SIZE_W, 0., TEXTURE_SIZE_W, TEXTURE_SIZE_H)))
    {
        pobSprite->autorelease();
        return pobSprite;
    }
