...
void TimeGoldCoinSprite::move(float x, float y) {
    ...
    this->runAction(CCSequence::create(
        CCSpawn::create(
            CCEaseIn::create(
                CCMoveTo::create(TIME_GOLD_COIN_MOVING_TERM, ccp(currentX, y)),
                2.),
            CCEaseOut::create(
                CCMoveTo::create(TIME_GOLD_COIN_MOVING_TERM, ccp(x, currentY)),
                2.),
            CCScaleTo::create(TIME_GOLD_COIN_MOVING_TERM, 1.), NULL),
