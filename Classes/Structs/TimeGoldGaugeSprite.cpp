...
TimeGoldGaugeSprite* TimeGoldGaugeSprite::createWithCustom()
{
    ...
    CCNotificationCenter::sharedNotificationCenter()->addObserver(pobSprite, callfuncO_selector(TimeGoldGaugeSprite::nabuzaTimeDoneCallback), "NABUZA_TIME_DONE_NOTIFICATION", NULL);
    CCNotificationCenter::sharedNotificationCenter()->addObserver(pobSprite, callfuncO_selector(TimeGoldGaugeSprite::nabuzaTimeFinishCallback), "NABUZA_TIME_FINISH_NOTIFICATION", NULL);
    
    if (pobSprite && pobSprite->initWithFile("coin_side_all.png", CCRect(0.,0.,TIME_GOLD_GAUGE_WIDTH,TIME_GOLD_GAUGE_HEIGHT)))  // 코인 한개당 가로 6
    {
...
void TimeGoldGaugeSprite::start() { // 1초에 2개씩 줄어든다.
    ...
    CCLog("Time Gold Gauge coinDelay : %f, tickerPerSecond : %u", coinDelay, this->tickPerSecond);
...
void TimeGoldGaugeSprite::addValue(unsigned int v) {
    ...
    if (changed > MAX_GAUGE_COUNT || changed == MAX_GAUGE_COUNT) {
        // 복주머니 처리하고 0으로...
        CCNotificationCenter::sharedNotificationCenter()->postNotification("TIME_GOLD_GAUGE_MAX_NOTIFICATION");
...
//    float xOffset    = TIME_GOLD_GAUGE_WIDTH - TIME_GOLD_GAUGE_COIN_WIDTH * v;
//    this->setTextureRect(CCRect(xOffset, 0., TIME_GOLD_GAUGE_WIDTH, TIME_GOLD_GAUGE_HEIGHT));
