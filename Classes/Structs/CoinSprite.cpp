...
CoinSprite* CoinSprite::createWithNumber(unsigned int key, unsigned int coinNumber)
{
    ...
    CCTexture2D *coinEffectTexture  = CCTextureCache::sharedTextureCache()->textureForKey("coin_all_star_effect.png");
    pobSprite->effectSprite = CCSprite::createWithTexture(coinEffectTexture, CCRect(0., 0., 156., 156.));
    pobSprite->effectSprite->setVisible(false);
    pobSprite->isNabuzaTime = false;
    
    CCNotificationCenter::sharedNotificationCenter()->addObserver(pobSprite, callfuncO_selector(CoinSprite::nabuzaTimeCallback), "NABUZA_TIME_NOTIFICATION", NULL);
    CCNotificationCenter::sharedNotificationCenter()->addObserver(pobSprite, callfuncO_selector(CoinSprite::nabuzaTimeDoneCallback), "NABUZA_TIME_DONE_NOTIFICATION", NULL);
    CCNotificationCenter::sharedNotificationCenter()->addObserver(pobSprite, callfuncO_selector(CoinSprite::nabuzaTimeFinishCallback), "NABUZA_TIME_FINISH_NOTIFICATION", NULL);
    
    if (pobSprite && pobSprite->initWithTexture(coinTexture, CCRect(coinNumber*MATRIX_COIN_WH,0.,MATRIX_COIN_WH,MATRIX_COIN_WH)))
    {
...
void CoinSprite::nabuzaTimeCallback(CCObject* obj) {    // 회전 속도 2배
    ...
    CCAnimation *rollingAnimation   = CCAnimationCache::sharedAnimationCache()->animationByName(animationCacheName);
    ...
    // CoinEffect started
    CCAnimation *coinEffectAnimation    = CCAnimationCache::sharedAnimationCache()->animationByName("coin_yellow_effect_animation");
    CCAnimate *aniCoinEffectAnimation   = CCAnimate::create(coinEffectAnimation);
    
    CCRepeatForever *coinEffectRepeat = CCRepeatForever::create(aniCoinEffectAnimation);
...
void CoinSprite::setIsNabuzaTime(bool isNabuzaTime, bool isAnimation) {
    ...
    if (isNabuzaTime == true) {
        this->setTextureRect(CCRect(rect.origin.x, 264., rect.size.width, rect.size.height));
        return;
    }
    
    this->setTextureRect(CCRect(rect.origin.x, 0., rect.size.width, rect.size.height));
}
void CoinSprite::setIsNabuzaTime(bool isNabuzaTime) {
    ...
    if (isNabuzaTime == true) {
        this->isNabuzaTime  = isNabuzaTime;
        int n; char animationCacheName[19];
        n   = sprintf(animationCacheName, "nabuza_animation_%u", this->number);
        CCAnimation *coinNabuzaAnimation    = CCAnimationCache::sharedAnimationCache()->animationByName(animationCacheName);
        this->runAction(
                        CCSequence::create(
                                           CCAnimate::create(coinNabuzaAnimation)
                                           ,CCCallFunc::create(this, callfunc_selector(CoinSprite::initializeFrame))
                                           ,NULL
                                           )
                        );
        return;
    ...
    int n; char animationCacheName[21];
...
void CoinSprite::initializeFrame() {
    if (this->isNabuzaTime) {
        this->setTextureRect(CCRect(MATRIX_COIN_WH*this->number, MATRIX_COIN_WH*4, MATRIX_COIN_WH, MATRIX_COIN_WH));
        return;
    }
    this->setTextureRect(CCRect(MATRIX_COIN_WH*this->number, 0., MATRIX_COIN_WH, MATRIX_COIN_WH));
    return;
}
...
void CoinSprite::setIsRolling(bool isRolling) {
    ...
    if (isRolling == true) {
        ...
        // CoinEffect started
        CCAnimation *coinEffectAnimation    = CCAnimationCache::sharedAnimationCache()->animationByName("coin_white_effect_animation");
        CCAnimate *aniCoinEffectAnimation   = CCAnimate::create(coinEffectAnimation);
...
#define COIN_ANIMATION_HEIGHT   500
    ...
    this->setPosition(ccp(originalPosition.x, originalPosition.y + COIN_ANIMATION_HEIGHT));
...
void CoinSprite::down() {
    ...
    if (false) {
        ...
        this->effectSprite->setPosition(ccp(position.x, position.y - MATRIX_CELL_WH));
    } else {
        CCActionInterval* actionMove  = CCMoveTo::create(0.1, ccp(position.x, position.y - MATRIX_CELL_WH));
        CCEaseIn* actionEaseMove    = CCEaseIn::create(actionMove, .2);
        this->runAction(CCSequence::create(actionEaseMove,
...
void CoinSprite::reset() {
    this->setTextureRect(CCRect(this->number*MATRIX_CELL_WH, 0., MATRIX_CELL_WH, MATRIX_CELL_WH));
...
void CoinSprite::beTimeGoldSprite(unsigned int goldKey) {
    ...
    this->runAction(CCSequence::create(
                                       ...
                                       ,CCCallFunc::create(this, callfunc_selector(CoinSprite::changeTimeGoldSprite))
                                       ,NULL
                    ));
}
...
void CoinSprite::changeTimeGoldSprite() {
    ...
    CCSprite *mustCoinCountSprite = CCSprite::create("play_gold_x.png", CCRect(mustTapCoinCount*15., 0., 15., 16.));
