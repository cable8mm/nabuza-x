...
ItemSprite* ItemSprite::createWithItemKey(unsigned int key) {
    ...
    if (pobSprite && pobSprite->initWithFile("item_choice_btn.png", CCRect(itemChoiceRectX, itemChoiceRectY, TEXTURE_SIZE_W, TEXTURE_SIZE_H)))
    {
...
void ItemSprite::setIsActive(bool isActive) {
    ...
    itemButtonRect.origin.x   = (this->isActive == true)? itemButtonRect.origin.x + TEXTURE_SIZE_W : itemButtonRect.origin.x - TEXTURE_SIZE_W;
