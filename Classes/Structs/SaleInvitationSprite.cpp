...
SaleInvitationSprite* SaleInvitationSprite::createWithItemKey(
    unsigned int key) {
    SaleInvitationSprite* pobSprite = new SaleInvitationSprite;
    pobSprite->key = key;
    if (pobSprite &&
        pobSprite->initWithFile(
            "sale_btn_invitation.png",
            CCRect(0., key * TEXTURE_SIZE_H, TEXTURE_SIZE_W, TEXTURE_SIZE_H))) {
...
SaleInvitationSprite* SaleInvitationSprite::createWithItemKeyTapped(
    unsigned int key) {
    SaleInvitationSprite* pobSprite = new SaleInvitationSprite;
    pobSprite->key = key;
