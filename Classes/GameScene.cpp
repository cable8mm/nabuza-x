...
//    this->addChild(gameLoadingLayer, LAYER_GAME_MESSAGE);
    ...
    CCNotificationCenter::sharedNotificationCenter()->addObserver(this, callfuncO_selector(GameScene::resume), "RESUME_NOTIFICATION", NULL);
    CCNotificationCenter::sharedNotificationCenter()->addObserver(this, callfuncO_selector(GameScene::restart), "RESTART_NOTIFICATION", NULL);
    CCNotificationCenter::sharedNotificationCenter()->addObserver(this, callfuncO_selector(GameScene::quit), "QUIT_NOTIFICATION", NULL);
    CCNotificationCenter::sharedNotificationCenter()->addObserver(this, callfuncO_selector(GameScene::timeGoldGaugeMaxNotification), "TIME_GOLD_GAUGE_MAX_NOTIFICATION", NULL);
    CCNotificationCenter::sharedNotificationCenter()->addObserver(this, callfuncO_selector(GameScene::notiGamePlaying), "NABUZA_PLAYING_NOTIFICATION", NULL);
    CCNotificationCenter::sharedNotificationCenter()->addObserver(this, callfuncO_selector(GameScene::notiGameLastChance), "NABUZA_LASTCHANCE_NOTIFICATION", NULL);

    // TIME_OVER_LAYER_DONE_NOTIFICATION
    CCNotificationCenter::sharedNotificationCenter()->addObserver(this, callfuncO_selector(GameScene::notiTimeOverDone), "TIME_OVER_LAYER_DONE_NOTIFICATION", NULL);
    // NABUZA_END_GAMERESULT_NOTIFICATION
    CCNotificationCenter::sharedNotificationCenter()->addObserver(this, callfuncO_selector(GameScene::notiEndGameResult), "NABUZA_END_GAMERESULT_NOTIFICATION", NULL);
    // NABUZA_END_LUCKY7RANKING_NOTIFICATION
    CCNotificationCenter::sharedNotificationCenter()->addObserver(this, callfuncO_selector(GameScene::notiEndLucky7Ranking), "NABUZA_END_LUCKY7RANKING_NOTIFICATION", NULL);
    // NABUZA_END_GAMEEVENT_NOTIFICATION
    CCNotificationCenter::sharedNotificationCenter()->addObserver(this, callfuncO_selector(GameScene::notiEndGameEvent), "NABUZA_END_GAMEEVENT_NOTIFICATION", NULL);
    CCLog("Start Game");
    ...
    playBgMidTimeSprite->setPosition(ccp(visibleSize.width / 2., 800. - 118 - 30.));
    ...
    playBgMidSprite->setPosition(ccp(visibleSize.width / 2., 800. - 108 - 332.));
    ...
    CCMenuItem* btnPlayPause    = CCMenuItemImage::create("play_btn_pause.png", "play_btn_pause.png", this, menu_selector(GameScene::pauseCallback));
    ...
    if (selectedItems->count() != 0) {
        for (unsigned int i=0; i<selectedItems->count(); i++) {
            CCInteger *itemKey = dynamic_cast<CCInteger*>(selectedItems->objectAtIndex(i));
            CCLog("Item Number i : %u, itemKey : %i", i, itemKey->getValue());

            CCSprite *playItemSprite    = CCSprite::create("play_item.png", CCRect(44.*itemKey->getValue(), 0., 44., 44.));
    ...
    CCString *strRemainSconds = CCString::createWithFormat("%d",this->remainSeconds);
    
    this->lbRemainSeconds  = CCLabelBMFont::create(strRemainSconds->getCString(), "play_time.fnt");
...
void GameScene::resume(CCObject* obj) {    // 재시작은 아이템 선택 화면으로 나가는 것을 의미
    CCLog("RESUME");
    CCDirector::sharedDirector()->resume();
//    this->resumeSchedulerAndActions();
}

void GameScene::restart(CCObject* obj) {    // 재시작은 아이템 선택 화면으로 나가는 것을 의미
    CCLog("RESTART");
    this->resume(obj);
}

void GameScene::quit(CCObject* obj) {    // 재시작은 아이템 선택 화면으로 나가는 것을 의미
    CCLog("QUIT");
    CCDirector::sharedDirector()->resume();
...
void GameScene::timeGoldGaugeMaxNotification(unsigned int goldKey) {   // 랜덤하게
    ...
    // 애니메이션
    TimeGoldCoinSprite *timeGoldCoinSprite  = TimeGoldCoinSprite::createWithColor(goldKey);   // 0은 금, 1은 은.
...
void GameScene::setGameScore(int score) {
    ...
    CCLabelBMFont *lbScore  = dynamic_cast<CCLabelBMFont*>(this->getChildByTag(TAG_GAME_SCORE_SPRITE));
...
void GameScene::setNabuzaTimeCount(unsigned int v) {
    ...
    if (v == 1) {
        for (unsigned int i=0; i<this->coinSprites->count(); i++) {
            CoinSprite *coinSprite  = dynamic_cast<CoinSprite*>(this->coinSprites->objectAtIndex(i));
...
void GameScene::setGameState(eGameStats kGameState) {
    ...
    switch (kGameState) {
        ...
        {
            GameLoadingLayer *gameLoadingLayer  = GameLoadingLayer::create();
            this->addChild(gameLoadingLayer, ZORDER_PULL_SCREEN, TAG_GAME_LOADING_LAYER);
            std::stringstream ss;
            ss  << "&spent_jade_count=" << NeedForGameMemory::sharedNeedForGameMemory()->getJadeCount();
            ss << "&spent_gold=" << NeedForGameMemory::sharedNeedForGameMemory()->getCardCount();
            ss << "&last_invitation_count_updated=" << UserStorage::sharedUserStorage()->getLastCardCountUpdated();
//            UserStorage::sharedUserStorage()->setCardCount(UserStorage::sharedUserStorage()->getCardCount()-1);
            ss << "&invitation_count=" << UserStorage::sharedUserStorage()->getCardCount();

            AnytaleHTTP::onHttpRequesting(this, callfuncND_selector(GameScene::onHttpRequestCompleted), "/players/start_game2", ss.str().c_str(), "PLAYERS_START_GAME2");
        }
            break;
        case kGAMESTATE_READY:
        {
            CCLog("kGAMESTATE_READY");
            GameLoadingLayer *gameLoadingLayer  = (GameLoadingLayer*)this->getChildByTag(TAG_GAME_LOADING_LAYER);
            gameLoadingLayer->removeFromParentAndCleanup(true);
            
            GameReadyStartLayer *gameReadyStartLayer    = GameReadyStartLayer::create();
        ...
        case kGAMESTATE_PLAYING:
        {
            ...
            this->schedule(schedule_selector(GameScene::tiktok), 1., this->remainSeconds, 0.);
            
            PlayTimeProgressSprite *playTimeProgressSprite  = PlayTimeProgressSprite::createWithCustom(this->remainSeconds);
            playTimeProgressSprite->setPosition(ccp(visibleSize.width / 2., 130.));
...
//            CCDirector::sharedDirector()->resume();
        ...
        {
            GameResultLayer *gameResultLayer    = GameResultLayer::createWithGameScore(this->gameScore);
            this->addChild(gameResultLayer, ZORDER_PULL_SCREEN);
        }
            break;
        case kGAMESTATE_LUCKY7RANKING:
        {
            Lucky7RankingLayer *lucky7RankingLayer    = Lucky7RankingLayer::createWithGameScore(this->gameScore);
        ...
        case kGAMESTATE_GAMEEVENT:
        {
            ...
            GameEventLayer *gameEventLayer  = GameEventLayer::createWithEventResult(eventResult);
...
void GameScene::onHttpRequestCompleted(CCHttpClient *sender, CCHttpResponse *response) {
    Json::Value root;
    ...
    if(strcmp(response->getHttpRequest()->getTag(), "PLAYERS_START_GAME2") == 0) {
        ...
        UserStorage::sharedUserStorage()->setCardCount(root["result"]["Player"].get("invitation_count", 0).asInt());
        UserStorage::sharedUserStorage()->setOwnGold(root["result"]["Player"].get("own_gold", 0).asInt());
...
unsigned int GameScene::getAddingGameScore() {
    ...
    if (this->nabuzaTimeCount > 0 || this->nabuzaTimeCount > this->needCoinCountForNabuzaTime) {
...
void GameScene::ccTouchEnded(CCTouch *touch, CCEvent *event) {
    ...
    unsigned int i = 0;
    do {
        CoinSprite* coinSprite  = (CoinSprite *)this->coinSprites->objectAtIndex(i);
        ...
        if (coinSprite->isRolling == true) {
            ...
            if (this->rollingCoinCount < this->needCoinCountForNabuzaTime) {
                ...
                coinSprite->runAction(CCSequence::create(
                                                      CCMoveTo::create(.4, this->timeGoldSprite->getPosition())
                                                         ,CCCallFuncN::create(this, callfuncN_selector(GameScene::removeCoin))
                                                         , NULL
                                      ));
            }
            ...
            for (unsigned int i = 0; i < coinSpriteCount; i++) {
                CoinSprite* coinSprite4Moving  = dynamic_cast<CoinSprite*>(this->coinSprites->objectAtIndex(i));
                intPoint enumCoinXY  = GetXYFromKey(coinSprite4Moving->key);
                
                if (enumCoinXY.y > pangCoinXY.y && enumCoinXY.x == pangCoinXY.x) {  // 팡되는 코인 위의 것 이라면...
                    if (enumCoinXY.y == 6) {   // 위에서 첫번째 줄이 떨어졌을 경우 혹은 첫번째 줄이 터졌을 경우, 첫번째 줄에 새로운 코인을 넣는다.
...
//    }
    ...
    TouchScoreSprite *touchScore    = TouchScoreSprite::createWithXY(pt.x, pt.y, addGameScore);
    ...
    if (this->rollingCoinCount > 8) {
        ...
        // 게이지 조정하고
        if (this->nabuzaTimeCount > 0) {
            bool isNabuzaTimeEnd    = this->timeNabuzaGaugeSprite->addValueFromRollingCount(this->rollingCoinCount);
...
//    CCTouch *touch  = (CCTouch*)*touches->begin();
    ...
    for (unsigned int i = 0; i < this->coinSprites->count(); i++) {
        CoinSprite *enumCoinSprite  = (CoinSprite *)this->coinSprites->objectAtIndex(i);
    ...
    if (currentSelectingCoinSprite->key == this->previousSelectingCoinSprite->key) {
        return;
    }
    
    if (this->selectingCoinNumber != currentSelectingCoinSprite->number && currentSelectingCoinSprite->number < 5) {
        return;
    }

    if (currentSelectingCoinSprite->number > 4 && currentSelectingCoinSprite->mustTapCoinCount > this->rollingCoinCount) {
        return;
    }
    ...
    if (currentSelectingCoinSprite->number == 6) {  // 은색 골드 코인
        for (unsigned int i = 0; i < this->coinSprites->count(); i++) {
            ...
            if ( enumCoinSprite->number < 5) {
                if (
                    (abs(enumSpritePoint.y-tapXY.y) == 1 && abs(enumSpritePoint.x-tapXY.x) == 1)
                    || (abs(enumSpritePoint.y-tapXY.y) == 1 && abs(enumSpritePoint.x-tapXY.x) == 0)
                    || (abs(enumSpritePoint.y-tapXY.y) == 0 && abs(enumSpritePoint.x-tapXY.x) == 1)
                    ) {
                    enumCoinSprite->setIsRolling(true);
                }
            }
    ...
    this->previousSelectingCoinSprite    = this->currentSelectingCoinSprite;  // this code must be located in the last line of ccTouchesMoved method.
}
...
//    CCTouch *touch  = (CCTouch*)*touches->begin();
    ...
    for (unsigned int i = 0; i < this->coinSprites->count(); i++) {
        ...
        if (enumCoinSprite->key == kCurrentSelectingCoinSprite) {
            ...
            this->previousSelectingCoinSprite    = this->currentSelectingCoinSprite = enumCoinSprite;
            this->isValidTouchBegan = false;
            break;
        }
    }
...
bool GameScene::canRolling(intPoint currentPoint) {    // 8방향에서 rolling 되는 코인이 존재 && 그 코인이 같은 색깔
    ...
    for (unsigned int i = 0; i < 8; i++) {
        ...
        CCLog("CurrentSelectingCoinNumber %u",this->currentSelectingCoinSprite->number );
        if (isValidXY(x, y)
            && this->coinsMatrix[x][y] == true
            && (this->currentSelectingCoinSprite->number == this->selectingCoinNumber
                || this->currentSelectingCoinSprite->number > 4
                )) {  // 12시 방향에서 시계방향으로 가자.
            return true;
        }
    }
...
CCPoint GetCCPFromKey(unsigned int key) {
    ...
    return ccp(coordinationX * MATRIX_CELL_WH + MATRIX_LEFT_MARGIN + MATRIX_CELL_WH/2, coordinationY * MATRIX_CELL_WH + MATRIX_BOTTOM_MARGIN + MATRIX_CELL_WH/2);
}

bool isValidXY(intPoint xy) {
    if (xy.x > -1 && xy.x < MATRIX_XY_COIN_COUNT && xy.y > -1 && xy.y < MATRIX_XY_COIN_COUNT) {
        return true;
    }
    return false;
}
...
bool isValidXY(int x, int y) {
    if (x > -1 && x < MATRIX_XY_COIN_COUNT && y > -1 && y < MATRIX_XY_COIN_COUNT) {
        return true;
    }
    return false;
}
...
bool isValidSelectingXY(intPoint previousPoint, intPoint currentPoint) {
    ...
    unsigned int previousNormalizedY    = (previousPoint.y % MATRIX_XY_COIN_COUNT);   // 0 - 6
    unsigned int currentNormalizedX     = (currentPoint.x % MATRIX_XY_COIN_COUNT);
    unsigned int currentNormalizedY     = (currentPoint.y % MATRIX_XY_COIN_COUNT);     // 0 - 6
    
    if (abs(previousNormalizedX - currentNormalizedX) == 1 && abs(previousNormalizedY - currentNormalizedY) == 1) {
        CCLog("is valid");
        return true;
    }
    CCLog("is not valid");
...
bool isValidSelectingXY(unsigned int kPreviousPoint, unsigned int kCurrentPoint) {
    ...
    unsigned int previousNormalizedY    = (previousXY.y % MATRIX_XY_COIN_COUNT);   // 0 - 6
    unsigned int currentNormalizedX     = (currentXY.x % MATRIX_XY_COIN_COUNT);
    unsigned int currentNormalizedY     = (currentXY.y % MATRIX_XY_COIN_COUNT);     // 0 - 6
    
    if (abs(previousNormalizedX - currentNormalizedX) < 2 && abs(previousNormalizedY - currentNormalizedY) < 2) {
        CCLog("is valid");
        return true;
    }
    CCLog("is not valid");
...
int getMultiplyConstant(unsigned int coinCount) {
    ...
    int constants[18]  = {-20, 6, 7, 8, 9, 10, 11, 13, 15, 17, 19, 21, 24, 27, 30, 33, 36, 40};
...
void playSoundJing(unsigned int rollingCoinCount) {
    ...
    CocosDenshion::SimpleAudioEngine::sharedEngine()->playEffect(ss.str().c_str());
}
...
void playSoundDropCoins(unsigned int coinCount) {
    if (coinCount < 4) {
        CocosDenshion::SimpleAudioEngine::sharedEngine()->playEffect("CD001.mp3");
        return;
    }
    
    if (coinCount < 9) {
        CocosDenshion::SimpleAudioEngine::sharedEngine()->playEffect("CD002.mp3");
        return;
    }
...
void playBgGamePlay() {
    ...
    CocosDenshion::SimpleAudioEngine::sharedEngine()->playBackgroundMusic("BG004.mp3", true);
    return;
}

void playBgGameNabuzaPlay() {
    CocosDenshion::SimpleAudioEngine::sharedEngine()->playBackgroundMusic("BG003.mp3", true);
    return;
}
