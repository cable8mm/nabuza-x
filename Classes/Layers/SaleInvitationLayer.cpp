...
void SaleInvitationLayer::onEnter() {
    ...
    AlertTitleSprite *alertTitleSprite =
        AlertTitleSprite::createWithItemKey(kGAMEALERTTITLE_BUY_CARD);
    ...
    //    this, menu_selector(SaleInvitationLayer::close)); CCMenu *menu    =
    //    CCMenu::create(closeButtonItem, NULL); menu->alignItemsHorizontally();
    //    closeButtonSprite->setPosition(ccp(visibleSize.width / 2,
    //    800.-648.-44.));
    this->addChild(closeButtonSprite);

    // 초대장 리스트
    SaleInvitationSprite *jade10ButtonSprite =
        SaleInvitationSprite::createWithItemKey(0);
    SaleInvitationSprite *jade10ButtonSpriteTapped =
        SaleInvitationSprite::createWithItemKeyTapped(0);
    CCMenuItemSprite *jade10ButtonItem = CCMenuItemSprite::create(
        jade10ButtonSprite, jade10ButtonSpriteTapped, this,
        menu_selector(SaleInvitationLayer::buyInvitation));
    jade10ButtonItem->setTag(0);

    SaleInvitationSprite *jade55ButtonSprite =
        SaleInvitationSprite::createWithItemKey(1);
    SaleInvitationSprite *jade55ButtonSpriteTapped =
        SaleInvitationSprite::createWithItemKeyTapped(1);
    CCMenuItemSprite *jade55ButtonItem = CCMenuItemSprite::create(
        jade55ButtonSprite, jade55ButtonSpriteTapped, this,
        menu_selector(SaleInvitationLayer::buyInvitation));
    jade55ButtonItem->setTag(1);

    SaleInvitationSprite *jade120ButtonSprite =
        SaleInvitationSprite::createWithItemKey(2);
    SaleInvitationSprite *jade120ButtonSpriteTapped =
        SaleInvitationSprite::createWithItemKeyTapped(2);
    CCMenuItemSprite *jade120ButtonItem = CCMenuItemSprite::create(
        jade120ButtonSprite, jade120ButtonSpriteTapped, this,
        menu_selector(SaleInvitationLayer::buyInvitation));
    jade120ButtonItem->setTag(2);

    SaleInvitationSprite *jade360ButtonSprite =
        SaleInvitationSprite::createWithItemKey(3);
    SaleInvitationSprite *jade360ButtonSpriteTapped =
        SaleInvitationSprite::createWithItemKeyTapped(3);
    CCMenuItemSprite *jade360ButtonItem = CCMenuItemSprite::create(
        jade360ButtonSprite, jade360ButtonSpriteTapped, this,
        menu_selector(SaleInvitationLayer::buyInvitation));
    jade360ButtonItem->setTag(3);

    SaleInvitationSprite *jade675ButtonSprite =
        SaleInvitationSprite::createWithItemKey(4);
    SaleInvitationSprite *jade675ButtonSpriteTapped =
        SaleInvitationSprite::createWithItemKeyTapped(4);
    CCMenuItemSprite *jade675ButtonItem = CCMenuItemSprite::create(
        jade675ButtonSprite, jade675ButtonSpriteTapped, this,
        menu_selector(SaleInvitationLayer::buyInvitation));
    jade675ButtonItem->setTag(4);

    CCMenu *jadeMenu =
        CCMenu::create(jade10ButtonItem, jade55ButtonItem, jade120ButtonItem,
                       jade360ButtonItem, jade675ButtonItem, NULL);
...
void SaleInvitationLayer::buyInvitation(CCObject *pSender) {
    ...
    if (invitationProducts[tag][1] >
        UserStorage::sharedUserStorage()->getJadeCount()) {
    ...
    AnytaleHTTP::onHttpRequesting(
        this, callfuncND_selector(SaleInvitationLayer::onHttpRequestCompleted),
        "/spent_jades/buy_invitation2", ss.str().c_str(),
        "SPENT_JADES_BUY_INVITATION2");
    IndicationLayer *indicationLayer = IndicationLayer::create();
    this->addChild(indicationLayer, ZORDER_INDICATION_LAYER,
                   TAG_INDICATION_LAYER);
}
...
void SaleInvitationLayer::onHttpRequestCompleted(CCHttpClient *sender,
    ...
    if (strcmp(response->getHttpRequest()->getTag(),
        ...
        int buyedCount =
            root["result"]["SpentJade"].get("buyed_count", 0).asInt();
...
bool SaleInvitationLayer::ccTouchBegan(CCTouch *pTouch, CCEvent *pEvent) {
    ...
    CCSprite *closeButtonSprite =
        (CCSprite *)this->getChildByTag(TAG_CLOSE_BUTTON_SPRITE);
