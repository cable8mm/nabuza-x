...
GameBgLayer::GameBgLayer(void) : isNabuzaTime(false) {
}
...
bool GameBgLayer::init() {
    ...
    CCTexture2D *playBgTopTexture =
        CCTextureCache::sharedTextureCache()->textureForKey(
            "play_bg_n_top.png");
    this->playBgTopSprite = CCSprite::createWithTexture(
        playBgTopTexture, CCRect(0., 0., 480., 118.));
    this->playBgTopSprite->setPosition(ccp(visibleSize.width / 2., 800. - 59.));
    this->addChild(this->playBgTopSprite);

    CCTexture2D *playBgBottomTexture =
        CCTextureCache::sharedTextureCache()->textureForKey(
            "play_bg_n_bottom.png");
    this->playBgBottomSprite = CCSprite::createWithTexture(
        playBgBottomTexture, CCRect(0., 0., 480., 118.));
...
void GameBgLayer::update() {
    CCTexture2D *playNabuzaDragonTexture =
        CCTextureCache::sharedTextureCache()->textureForKey(
            "play_n_time_dragon.png");
    CCSprite *playNabuzaDragonSprite = CCSprite::createWithTexture(
        playNabuzaDragonTexture, CCRect(0., 0., 246., 469.));
    float randomX = (float)(rand() % 300) + 50.;
    float randomY = (float)(rand() % 50);

    playNabuzaDragonSprite->setPosition(ccp(randomX, -200. - randomY));

    playNabuzaDragonSprite->runAction(CCRepeatForever::create(CCAnimate::create(
    ...
    playNabuzaDragonSprite->runAction(CCSequence::create(
        ...
        CCCallFuncN::create(
            this, callfuncN_selector(GameBgLayer::removeNabuzaSprite)),
        NULL));
...
void GameBgLayer::setIsNabuzaTime(bool yorn) {
    ...
    if (this->isNabuzaTime == true) {
        this->runAction(CCRepeatForever::create(CCSequence::create(
            ...
            CCDelayTime::create(.5), NULL)));

        this->playBgTopSprite->runAction(CCSpeed::create(
            CCRepeatForever::create(CCAnimate::create(
                CCAnimationCache::sharedAnimationCache()->animationByName(
                    "nabuza_b_bg_animation"))),
            .5));

        this->playBgBottomSprite->runAction(CCSpeed::create(
            CCRepeatForever::create(CCAnimate::create(
                CCAnimationCache::sharedAnimationCache()->animationByName(
                    "nabuza_b_bg_animation"))),
            .5));
