...
bool BaseRankingLayer::init()
{
    ...
    CCNotificationCenter::sharedNotificationCenter()->addObserver(this, callfuncO_selector(BaseRankingLayer::changeToSelectItemsLayer), "NABUZA_SELECT_ITEMS_NOTIFICATION", NULL);
    CCNotificationCenter::sharedNotificationCenter()->addObserver(this, callfuncO_selector(BaseRankingLayer::changeToRankingLayer), "NABUZA_RANKING_NOTIFICATION", NULL);
//    CCNotificationCenter::sharedNotificationCenter()->addObserver(this, callfuncO_selector(BaseRankingLayer::onBuyJadeLayer), "ON_BUY_JADE_LAYER_NOTIFICATION", NULL);
...
//
//        for (unsigned int i=0; i < this->itemSelectionLayer->itemChoosedSprites->count(); i++) {
//            ItemChoosedSprite *itemChoosedSprite    = dynamic_cast<ItemChoosedSprite*>(this->itemSelectionLayer->itemChoosedSprites->objectAtIndex(i));
...
//    
//    // 플러스맥스스프라이트
//    PlusMaxSprite *plusMaxSprite    = (PlusMaxSprite *)this->getChildByTag(TAG_PLUSMAX_SPRITE);
...
void BaseRankingLayer::onHttpRequestCompleted(CCHttpClient *sender, CCHttpResponse *response) {
    Json::Value root;
    ...
    if(strcmp(response->getHttpRequest()->getTag(), "EVENTS_GET") == 0) {
        ...
        for (int index=0; index < events.size(); index++) {
            std::string linkUrl = events[index]["Event"].get("link_url", "").asString();
            WebViewLayer *webViewLayer  = WebViewLayer::createWithUrl(linkUrl.c_str());
