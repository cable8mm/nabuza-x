...
void Lucky7RankingLayer::onEnter() {
    ...
    AlertTitleSprite *alertTitleSprite =
        AlertTitleSprite::createWithItemKey(kGAMEALERTTITLE_TOURNAMENT_RESULT);
    ...
    AnytaleHTTP::onHttpRequesting(
        this, callfuncND_selector(Lucky7RankingLayer::onHttpRequestCompleted),
        "/seven_tournament_volunteers/submit2", ss.str().c_str(),
        "SEVEN_TOURNAMENT_VOLUNTEERS_SUBMIT2");
...
void Lucky7RankingLayer::onHttpRequestCompleted(CCHttpClient *sender,
    ...
    if (strcmp(response->getHttpRequest()->getTag(),
               "SEVEN_TOURNAMENT_VOLUNTEERS_SUBMIT2") == 0) {
        /*

         {"code":0,"message":"OK","result":{"ranking_infos":{"ranking":1,"reward_jade_count":1,"reward_gold_count":0},"player":{"own_jade_count":33,"own_gold_count":7639},"rankings":[{"SevenTournamentVolunteer":{"id":172731,"player_id":32868,"nickname":"Player#32868","own_gold":6997,"highscore":1232123,"created":"2013-07-24
         15:43:07"}},{"SevenTournamentVolunteer":{"id":172719,"player_id":32709,"nickname":"\uc544\ub4e4\ub09c\ud2f0\ub098","own_gold":1408,"highscore":709971,"created":"2013-07-16
         18:25:43"}},{"SevenTournamentVolunteer":{"id":172716,"player_id":32863,"nickname":"\uc5c6\uc74c","own_gold":0,"highscore":962524,"created":"2013-07-16
         18:22:21"}},{"SevenTournamentVolunteer":{"id":172708,"player_id":32862,"nickname":"\uc591\ubbf8","own_gold":12457,"highscore":527670,"created":"2013-07-16
         18:15:13"}},{"SevenTournamentVolunteer":{"id":172583,"player_id":32797,"nickname":"\ub098\uc911\uc5d0","own_gold":17652,"highscore":218170,"created":"2013-07-16
         17:25:08"}},{"SevenTournamentVolunteer":{"id":172560,"player_id":32846,"nickname":"\ubc15\uc0ac\uc544\ub4e4","own_gold":23242,"highscore":621600,"created":"2013-07-16
         17:05:58"}}]}}
         */
        ...
        for (int index = 0; index < rankings.size(); index++) {
            SevenTournamentVolunteerModel *sevenTournamentVolunteer =
                new SevenTournamentVolunteerModel;
            sevenTournamentVolunteer->rank =
                rankings[index]["SevenTournamentVolunteer"]["rank"].asInt();
            sevenTournamentVolunteer->highscore =
                rankings[index]["SevenTournamentVolunteer"]["highscore"]
                    .asInt();
            sevenTournamentVolunteer->nickname =
                rankings[index]["SevenTournamentVolunteer"]["nickname"]
                    .asString();
            sevenTournamentVolunteer->ownGoldCount =
                rankings[index]["SevenTournamentVolunteer"]["own_gold"].asInt();
            sevenTournamentVolunteer->isMe =
                rankings[index]["SevenTournamentVolunteer"]["isMe"].asBool();
            this->sevenTournamentVolunteers->addObject(
                sevenTournamentVolunteer);
        }

        CCTableView *rankingTableView =
            CCTableView::create(this, CCSizeMake(400., 395.));
        ...
        CCSize visibleSize = CCDirector::sharedDirector()->getVisibleSize();
        // 닫기 버튼
        ButtonSprite *checkPrizeButtonSprite =
            ButtonSprite::createWithItemKey(kGAMEBUTTON_CONFIRM_REWARD);
        ButtonSprite *checkPrizeButtonSpriteTapped =
            ButtonSprite::createWithItemKey(kGAMEBUTTON_CONFIRM_REWARD);
        checkPrizeButtonSpriteTapped->setScale(1.2);
        checkPrizeButtonSpriteTapped->setPosition(
            ccp(checkPrizeButtonSprite->getPositionX() - 20.,
                checkPrizeButtonSprite->getPositionY() - 6.));

        CCMenuItemSprite *closeButtonItem = CCMenuItemSprite::create(
            checkPrizeButtonSprite, checkPrizeButtonSpriteTapped, this,
            menu_selector(Lucky7RankingLayer::close));
...
void Lucky7RankingLayer::close() {
    ...
    CCNotificationCenter::sharedNotificationCenter()->postNotification(
        "NABUZA_END_LUCKY7RANKING_NOTIFICATION", rank);
...
CCTableViewCell *Lucky7RankingLayer::tableCellAtIndex(CCTableView *table,
                                                      unsigned int idx) {
    SevenTournamentVolunteerModel *sevenTournamentVolunteer =
        (SevenTournamentVolunteerModel *)(this->sevenTournamentVolunteers
                                              ->objectAtIndex(idx));
    ...
    if (!cell) {
        ...
        if (cellBackgroundSpriteKey < 3) {
            cellBackgroundSprite->setTextureRect(
                CCRect(0., cellBackgroundSpriteKey * 80., 400., 80.));
        } else {
            cellBackgroundSprite->setTextureRect(
                CCRect(0., 3. * 80., 400., 80.));
        }
        ...
        // 닉네임
        CCLabelTTF *lbNickName =
            CCLabelTTF::create(sevenTournamentVolunteer->nickname.c_str(),
                               DEFAULT_FONT_NAME, 16.0);
        ...
        LevelIconSprite *levelIconSprite = LevelIconSprite::createWithGoldCount(
            sevenTournamentVolunteer->ownGoldCount);
        ...
        CCLabelTTF *highscoreSprite =
            CCLabelTTF::create(sHighscore.c_str(), DEFAULT_FONT_NAME, 18.0);
        ...
        CCLabelBMFont *rankLabel =
            (CCLabelBMFont *)cell->getChildByTag(TAG_RANK_LABEL);
        std::stringstream ss;
        ss << sevenTournamentVolunteer->rank;
        rankLabel->setString(ss.str().c_str());
        CCSprite *cellBackgroundSprite =
            (CCSprite *)cell->getChildByTag(TAG_BACKGROUND_SPRITE);
        ...
        // 닉네임
        CCLabelTTF *lbNickName =
            (CCLabelTTF *)cell->getChildByTag(TAG_NICKNAME_LABEL);
        ...
        ATUtil::insert_separator(sHighscore);
        CCLabelTTF *highscoreSprite =
            (CCLabelTTF *)cell->getChildByTag(TAG_HIGHSCORE_LABEL);
        highscoreSprite->setString(sHighscore.c_str());

        MyRankSprite *myRankSprite =
            (MyRankSprite *)cell->getChildByTag(TAG_MY_RANK_SPRITE);
...
CCSize Lucky7RankingLayer::cellSizeForIndex(CCTableView *table,
                                            unsigned int idx) {
...
void Lucky7RankingLayer::tableCellTouched(CCTableView *table,
