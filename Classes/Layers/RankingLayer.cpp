...
bool RankingLayer::init() {
    ...
    this->btnFBLoginSprite->setPosition(
        ccp(visibleSize.width / 2., 800. - 531. - 31.));
...
void RankingLayer::onHttpRequestCompleted(CCHttpClient *sender,
    ...
    if (strcmp(response->getHttpRequest()->getTag(),
               "MESSAGES_SEND_INVITATION2") == 0) {
        CCTableViewCell *cell =
            this->rankingTableView->cellAtIndex(this->sendInvitationCellIdx);
...
void RankingLayer::ccTouchEnded(CCTouch *pTouch, CCEvent *pEvent) {
    //    if (this->btnFBLoginSprite->boundingBox().containsPoint(pt)) {    //
    //    로그인
...
CCTableViewCell *RankingLayer::tableCellAtIndex(CCTableView *table,
                                                unsigned int idx) {
    CCTableViewCell *cell = table->dequeueCell();
    BuddyModel *buddy;
    if (idx != 10) {
        buddy = (BuddyModel *)(BuddyStorage::sharedBuddyStorage()
                                   ->buddies->objectAtIndex(idx));
    }
    ...
    if (!cell) {
        ...
        if (false) {
            ...
            // 닉네임
            CCLabelTTF *lbNickName = CCLabelTTF::create(
                buddy->nickname.c_str(), DEFAULT_FONT_NAME, 16.0);
        ...
        CCSprite *invitedBuddyBgSprite =
            (CCSprite *)cell->getChildByTag(TAG_INVITED_BUDDY_CELL);
        if (invitedBuddyBgSprite != NULL) {
            invitedBuddyBgSprite->setTextureRect(
                CCRect(0., 4. * 80., 400., 80.));
        }
    }
...
void RankingLayer::tableCellTouched(CCTableView *table, CCTableViewCell *cell) {
    ...
    BuddyModel *buddy =
        (BuddyModel *)(BuddyStorage::sharedBuddyStorage()
                           ->buddies->objectAtIndex(cell->getIdx()));
    ...
    if (this->tappedPoint.x > 370 && this->tappedPoint.x < 440) {  // 초대
        ...
        if (buddy->isInvitationNotification) {
            ...
            AnytaleHTTP::onHttpRequesting(
                this, callfuncND_selector(RankingLayer::onHttpRequestCompleted),
                "/messages/send_invitation2", ss.str().c_str(),
                "MESSAGES_SEND_INVITATION2");
        }
    } else {  // 버디 정보 플로팅
        BuddyInfoLayer *buddyInfoLayer =
            BuddyInfoLayer::createWithBuddyKey(cell->getIdx());
