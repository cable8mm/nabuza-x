...
BuddyInfoLayer *BuddyInfoLayer::createWithBuddyKey(int buddyKey) {
    BuddyInfoLayer *pobLayer = new BuddyInfoLayer;
    pobLayer->buddyModel    = (BuddyModel *)BuddyStorage::sharedBuddyStorage()->buddies->objectAtIndex(buddyKey);
...
void BuddyInfoLayer::onEnter()
{
    ...
    LevelInformation *levelInformation  = LevelInformation::sharedLevelInformation();
    levelInformation->setting(this->buddyModel->ownGoldCount);
    
    // 박스
    CCSprite *bgBoxSprite   = CCSprite::create("title_alert.png");
    bgBoxSprite->setPosition(ccp(visibleSize.width / 2, visibleSize.height / 2 + 25.));
    ...
    // BG 돈다발
    LevelMoneySprite *bgMoneySprite = LevelMoneySprite::createWithGoldCount(this->buddyModel->ownGoldCount);
    bgMoneySprite->setPosition(ccp(404., 800.-508));
    this->addChild(bgMoneySprite);
    
    // 이름
    CCLabelTTF *nameLabel    = CCLabelTTF::create(this->buddyModel->nickname.c_str(), DEFAULT_FONT_NAME, 30.);
    ...
    // 직책
    CCLabelTTF *gradeLabel  = CCLabelTTF::create(levelInformation->getDisplayName().c_str(), DEFAULT_FONT_NAME, 35);
...
//    CCLabelBMFont *levelLabel   = CCLabelBMFont::create("LV", "font_56_brown.fnt");
...
//    this->addChild(levelLabel);
    ...
    CCSprite *levelSprite   = CCSprite::create("level_no.png", CCRect(level%3*64., floorf(level/3)*40., 64., 40.));
    levelSprite->setAnchorPoint(ccp(1.,0.));
    levelSprite->setPosition(ccp(285., 800.-255.));
    this->addChild(levelSprite);
    
//    CCLabelBMFont *playerLevelLabel   = CCLabelBMFont::create("", "rst_score.fnt");
...
//    this->addChild(playerLevelLabel);
    ...
    CCLabelTTF *ownGoldLabel   = CCLabelTTF::create(ownGoldCount.c_str(), DEFAULT_FONT_NAME, 20);
    ...
    // 다음레벨까지
    std::string needGoldCount = GetStringFromInt(levelInformation->getNeedGoldCountForNextLevel());
    ATUtil::insert_separator(needGoldCount);
    CCLabelTTF *nextLevelLabel   = CCLabelTTF::create(needGoldCount.c_str(), DEFAULT_FONT_NAME, 20);
    ...
    // 레벨 보너스까지
    std::string levelBonus = GetStringFromInt(levelInformation->getLevelBonus());
    ATUtil::insert_separator(levelBonus);
    CCLabelTTF *levelBonusLabel   = CCLabelTTF::create(levelBonus.c_str(), DEFAULT_FONT_NAME, 20);
    ...
    // 금주 최고 점수
    std::string weeklyHighscore = GetStringFromInt(this->buddyModel->weeklyHighscore);
    ATUtil::insert_separator(weeklyHighscore);
    CCLabelTTF *thisWeekHighScoreLabel   = CCLabelTTF::create(weeklyHighscore.c_str(), DEFAULT_FONT_NAME, 20);
    ...
    CCLabelTTF *highScoreLabel   = CCLabelTTF::create(highscore.c_str(), DEFAULT_FONT_NAME, 20);
    ...
    // 메달 갯수
    std::string goldMedalCount = GetStringFromInt(this->buddyModel->ownGoldMedalCount);
    CCLabelBMFont *goldMedalLabel   = CCLabelBMFont::create(goldMedalCount.c_str(), "font_brown_45.fnt");
    ...
    std::string silverMedalCount = GetStringFromInt(this->buddyModel->ownSilverMedalCount);
    CCLabelBMFont *silverMedalLabel   = CCLabelBMFont::create(silverMedalCount.c_str(), "font_brown_45.fnt");
    ...
    std::string bronzeMedalCount = GetStringFromInt(this->buddyModel->ownBronzeMedalCount);
    CCLabelBMFont *bronzeMedalLabel   = CCLabelBMFont::create(bronzeMedalCount.c_str(), "font_brown_45.fnt");
    ...
    CCMenuItemImage *btnCloseItem   = CCMenuItemImage::create("exit_btn.png", "exit_btn.png", this, callfuncO_selector(BuddyInfoLayer::close));
...
void BuddyInfoLayer::onHttpRequestCompleted(CCHttpClient *sender, CCHttpResponse *response) {
    Json::Value root;
    Json::Reader reader;
    ...
    // 게임 시작을 서버에 알림
    if(strcmp(response->getHttpRequest()->getTag(), "PLAYERS_GET_BUDDY") == 0) {
        LevelInformation *levelInformation  = LevelInformation::sharedLevelInformation();
        levelInformation->setting(root["result"]["Player"].get("own_gold", 0).asInt());
...
//        int getNeedGoldCountForNextLevel();
        ...
        CCLabelTTF *nicknameLabel    = (CCLabelTTF *)this->getChildByTag(TAG_BUDDY_NAME_LABEL);
        nicknameLabel->setString(ss.c_str());

        CCLabelTTF *gradeLabel    = (CCLabelTTF *)this->getChildByTag(TAG_BUDDY_GRADE_NAME_LABEL);
        gradeLabel->setString(levelInformation->getDisplayName().c_str());
        
        ss = GetStringFromInt(root["result"]["Player"].get("highscore", 0).asInt());
        CCLabelTTF *highscoreLabel  = (CCLabelTTF *)this->getChildByTag(TAG_BUDDY_HIGHLIGHT_LABEL);
        highscoreLabel->setString(ss.c_str());

        ss = GetStringFromInt(root["result"]["Player"].get("own_gold", 0).asInt());
        CCLabelTTF *ownGoldLabel    = (CCLabelTTF *)this->getChildByTag(TAG_BUDDY_OWN_GOLD_LABEL);
        ownGoldLabel->setString(ss.c_str());
        
        ss = GetStringFromInt(levelInformation->getLevelBonus());
        CCLabelTTF *levelBonusLabel    = (CCLabelTTF *)this->getChildByTag(TAG_BUDDY_LEVEL_BONUS_LABEL);
        levelBonusLabel->setString(ss.c_str());

        ss = GetStringFromInt(levelInformation->getNeedGoldCountForNextLevel());
        CCLabelTTF *nextLevelLabel    = (CCLabelTTF *)this->getChildByTag(TAG_BUDDY_NEXT_LEVEL_LABEL);
        nextLevelLabel->setString(ss.c_str());
        
        ss = GetStringFromInt(root["result"]["Player"].get("gold_medal_count", 0).asInt());
        CCLabelBMFont *goldMedalCountLabel  = (CCLabelBMFont *)this->getChildByTag(TAG_BUDDY_GOLD_MEDAL_LABEL);
        goldMedalCountLabel->setString(ss.c_str());
        
        ss = GetStringFromInt(root["result"]["Player"].get("silver_medal_count", 0).asInt());
        CCLabelBMFont *silverMedalCountLabel  = (CCLabelBMFont *)this->getChildByTag(TAG_BUDDY_SILVER_MEDAL_LABEL);
        silverMedalCountLabel->setString(ss.c_str());
        
        ss = GetStringFromInt(root["result"]["Player"].get("bronze_medal_count", 0).asInt());
        CCLabelBMFont *bronzeMedalCountLabel  = (CCLabelBMFont *)this->getChildByTag(TAG_BUDDY_BRONZE_MEDAL_LABEL);
        bronzeMedalCountLabel->setString(ss.c_str());

        ss = GetStringFromInt(root["result"]["Player"].get("weekly_highscore", 0).asInt());
        CCLabelTTF *weeklyHighscoreLabel  = (CCLabelTTF *)this->getChildByTag(TAG_BUDDY_WEEKLY_HIGHLIGHT_LABEL);
        weeklyHighscoreLabel->setString(ss.c_str());

//        ss = GetStringFromInt(levelInformation->getLevel());
//        CCLabelBMFont *levelLabel  = (CCLabelBMFont *)this->getChildByTag(TAG_BUDDY_LEVEL_LABEL);
