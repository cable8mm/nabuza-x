...
void SendingGiftLayer::onEnter() {
    ...
    AlertTitleSprite *alertTitleSprite =
        AlertTitleSprite::createWithItemKey(kGAMEALERTTITLE_SENDING_GIFT);
    ...
    CCMenuItemSprite *closeButtonItem =
        CCMenuItemSprite::create(closeButtonSprite, closeButtonSpriteTapped,
                                 this, menu_selector(SendingGiftLayer::close));
    ...
    AnytaleHTTP::onHttpRequesting(
        this, callfuncND_selector(SendingGiftLayer::onHttpRequestCompleted),
        "/players/view2", "", "PLAYERS_VIEW2");

    CCLayerColor::onEnter();
}

void SendingGiftLayer::onHttpRequestCompleted(CCHttpClient *sender,
...
bool SendingGiftLayer::ccTouchBegan(CCTouch *touch, CCEvent *pEvent) {
    ...
    //    CCLog("CCTouchEnded Touch Location, x : %f, y : %f",
    //    this->tappedPoint.x, this->tappedPoint.y);
...
CCTableViewCell *SendingGiftLayer::tableCellAtIndex(CCTableView *table,
                                                    unsigned int idx) {
    GiftBuddyModel *buddy =
        (GiftBuddyModel *)(this->buddies->objectAtIndex(idx));
    ...
    if (!cell) {
        ...
        LevelIconSprite *levelIconSprite =
            LevelIconSprite::createWithGoldCount(buddy->ownGoldCount);
        ...
        // 닉네임
        CCLabelTTF *lbNickName = CCLabelTTF::create(buddy->nickname.c_str(),
        ...
        // x50개 선물하기
        CCLabelTTF *lbGiftMessage =
            CCLabelTTF::create("x50개 선물하기", DEFAULT_FONT_NAME, 18.0);
        ...
        // 초대 아이콘
        CCSprite *giftJadeIconSprite =
            CCSprite::create("jewel_send_box.png", CCRect(0., 0., 60., 60.));
        ...
        //        (CCLabelTTF*)this->getChildByTag(TAG_NICKNAME_LABEL);
...
CCSize SendingGiftLayer::cellSizeForIndex(CCTableView *table,
                                          unsigned int idx) {
...
void SendingGiftLayer::tableCellTouched(CCTableView *table,
                                        CCTableViewCell *cell) {
    CCLOG("cell touched at index: %i", cell->getIdx());
    GiftBuddyModel *buddy =
        (GiftBuddyModel *)(this->buddies->objectAtIndex(cell->getIdx()));
    ...
    if (this->tappedPoint.x > 370 && this->tappedPoint.x < 440) {
        if (UserStorage::sharedUserStorage()->getOwnJadeCountToInt() < 50) {
            JadeLessAlertViewLayer *jadeLessAlertViewLayer =
                JadeLessAlertViewLayer::createWithKey(kJADELESSKEY_NOT_GIFT);
        ...
        AnytaleHTTP::onHttpRequesting(
            this, callfuncND_selector(SendingGiftLayer::onHttpRequestCompleted),
            "/spent_gifts/send", ss.str().c_str(), "SPENT_GIFTS_SEND");
