...
bool BuyJadeLayer::init() {
    ...
    CCNotificationCenter::sharedNotificationCenter()->addObserver(
        this, callfuncO_selector(BuyJadeLayer::purchasingIAP),
        "PURCHASING_IAP_NOTIFICATION", NULL);
    CCNotificationCenter::sharedNotificationCenter()->addObserver(
        this, callfuncO_selector(BuyJadeLayer::purchasedIAP),
        "PURCHASED_IAP_NOTIFICATION", NULL);
    CCNotificationCenter::sharedNotificationCenter()->addObserver(
        this, callfuncO_selector(BuyJadeLayer::failedIAP),
        "FAILED_IAP_NOTIFICATION", NULL);
    CCNotificationCenter::sharedNotificationCenter()->addObserver(
        this, callfuncO_selector(BuyJadeLayer::restoredIAP),
        "RESTORED_IAP_NOTIFICATION", NULL);

    return true;
}

void BuyJadeLayer::purchasingIAP(CCObject *obj) {  // 서버에 거래 처리중
    //    IndicationLayer *indicatorLayer =
    //    (IndicationLayer*)this->getChildByTag(TAG_INDICATOR_LAYER);
...
void BuyJadeLayer::purchasedIAP(CCObject *obj) {  // 구매 완료
    ...
    AnytaleHTTP::onHttpRequesting(
        this, callfuncND_selector(BuyJadeLayer::onHttpRequestCompleted),
        "/purchasings/item2", ss.str().c_str(), "PURCHASINGS_ITEM2");
}
...
void BuyJadeLayer::failedIAP(CCObject *obj) {  // 거래 실패 또는 취소
    ...
    //
    //    std::stringstream ss;
    //    ss << productIdentifier->getCString() << " 이 구입되지
    //    않았습니다.\n다시 시도해 주세요.";
    //
...
void BuyJadeLayer::onHttpRequestCompleted(CCHttpClient *sender,
    ...
    if (strcmp(response->getHttpRequest()->getTag(), "PURCHASINGS_ITEM2") ==
        0) {
        IndicationLayer *indicatorLayer =
            (IndicationLayer *)this->getChildByTag(TAG_INDICATOR_LAYER);
        if (indicatorLayer != NULL) {
...
void BuyJadeLayer::onEnter() {
    ...
    AlertTitleSprite *alertTitleSprite =
        AlertTitleSprite::createWithItemKey(kGAMEALERTTITLE_BUY_JADE);
    this->addChild(alertTitleSprite);

    // 닫기 버튼
    ButtonSprite *couponButtonSprite =
        ButtonSprite::createWithItemKey(kGAMEBUTTON_COUPON_REGIST);
    ButtonSprite *couponButtonSpriteTapped =
        ButtonSprite::createWithItemKey(kGAMEBUTTON_COUPON_REGIST);
    couponButtonSpriteTapped->setScale(1.2);
    couponButtonSpriteTapped->setPosition(
        ccp(couponButtonSprite->getPositionX() - 20.,
            couponButtonSprite->getPositionY() - 6.));

    CCMenuItemSprite *couponButtonItem = CCMenuItemSprite::create(
        couponButtonSprite, couponButtonSpriteTapped, this,
        menu_selector(BuyJadeLayer::registCoupon));
    ...
    CCMenuItemSprite *closeButtonItem =
        CCMenuItemSprite::create(closeButtonSprite, closeButtonSpriteTapped,
                                 this, menu_selector(BuyJadeLayer::close));
    ...
    for (unsigned int i = 0; i < JADEBUTTON_COUNT; i++) {
        BuyJadeButtonSprite *jadeButtonSprite =
            BuyJadeButtonSprite::createWithItemKey(i);
        BuyJadeButtonSprite *jadeButtonSpriteTapped =
            BuyJadeButtonSprite::createWithItemKeyTapped(i);
        CCMenuItemSprite *jadeButtonItem = CCMenuItemSprite::create(
            jadeButtonSprite, jadeButtonSpriteTapped, this,
            menu_selector(BuyJadeLayer::onBuyJadeButtonTapped));
    ...
    CCLabelTTF *lbNabuzaInfomation = CCLabelTTF::create(
        "위 금액은 부가세 10%가 포함된 금액입니다.", "JejuGothic", 20);
...
void BuyJadeLayer::onBuyJadeButtonTapped(CCObject *object) {
    ...
    CCAssert(needJadeCountKey >= 0 && needJadeCountKey < JADEBUTTON_COUNT,
    ...
    //    callfuncND_selector(BuyGoldLayer::onHttpRequestCompleted),
