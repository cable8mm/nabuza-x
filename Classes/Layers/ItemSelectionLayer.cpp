...
//    bgRankingSprite->setPosition(ccp(visibleSize.width / 2., 695.));
    ...
    // 초보방
//    CCSprite *itemLevelSprite   = CCSprite::create("item_lv.png", CCRect(0., 0., 222., 52.));   // height:52
...
//    this->addChild(itemLevelSprite);
    ...
    itemBoardSprite->setPosition(ccp(visibleSize.width/2, 800.-(263.+63.5)));
    this->addChild(itemBoardSprite);
    
    // 아이템 인포
    ItemInfoSprite *itemInfoSprite    = ItemInfoSprite::create();
    itemInfoSprite->setPosition(ccp(visibleSize.width/2, 800.-(280.+44.)));
...
//#define TEXTURE_SIZE_H  88.
    ...
    CCTexture2D *bgItemCoosedTexture = CCTextureCache::sharedTextureCache()->addImage("item_empty_box_btn.png");
    for (unsigned int i = 0; i < 3; i++) {
        CCSprite *bgItemCoosedSprite  = CCSprite::createWithTexture(bgItemCoosedTexture);
        bgItemCoosedSprite->setPosition(ccp(41.+135.*i + 64.5, 800. - (161. + 42.5)));
...
//    CCTexture2D *itemChoiceds = CCTextureCache::sharedTextureCache()->addImage("item_box_btn.png");
//    for (unsigned int i = 0; i < 5; i++) {
//        ItemChoosedSprite *itemCoosedSprite  = ItemChoosedSprite::create();
...
//    }
    ...
    CCMenuItemImage *btnCloseItem   = CCMenuItemImage::create("exit_btn.png", "exit_btn.png", this, callfuncO_selector(ItemSelectionLayer::close));
    ...
    BelowStartLayer *belowStartLayer    = BelowStartLayer::createWithIsStartGame(true);
    this->addChild(belowStartLayer);
    
//    CardLayer *cardLayer    = CardLayer::create();
//    this->addChild(cardLayer);
    
//    BelowTapLayer *belowTapLayer    = BelowTapLayer::createWithIsStartGame(true);
...
bool ItemSelectionLayer::ccTouchBegan(CCTouch *pTouch, CCEvent *pEvent) {
    ...
    for (unsigned int i=0; i<ITEM_TOTAL_COUNT; i++) {
        ItemSprite *itemSprite  = dynamic_cast<ItemSprite*>(this->itemButtonSprites->objectAtIndex(i));
        if (itemSprite->boundingBox().containsPoint(pt) && itemSprite->getIsActive()) {
    ...
    for (unsigned int i=0; i<this->itemChoosedSprites->count(); i++) {
        ItemChoosedSprite *itemChoosedSprite    = dynamic_cast<ItemChoosedSprite*>(this->itemChoosedSprites->objectAtIndex(i));
...
void ItemSelectionLayer::ccTouchEnded(CCTouch *pTouch, CCEvent *pEvent) {
    ...
    if (this->selectedItemSprite != NULL) {
        for (unsigned int i=0; i<this->itemButtonSprites->count(); i++) {
            ItemSprite *itemButtonSprite  = dynamic_cast<ItemSprite*>(this->itemButtonSprites->objectAtIndex(i));
            if (itemButtonSprite->boundingBox().containsPoint(pt)) {
                ...
                if (this->selectedItemSprite == itemButtonSprite) {
                    ...
                    int kNextItemChoosedSprite  = GetNextItemChoosedSpriteKey(this->itemChoosedSprites);
                    ...
                    // 아이템 선택 버튼을 만들어서 배치
                    ItemChoosedSprite *itemChoosedSprite    = ItemChoosedSprite::createWithKeys(itemButtonSprite->getKey(), kNextItemChoosedSprite);
                    
                    CCLog("kNextItemChoosedSprite value : %i", kNextItemChoosedSprite);
                    ...
                    // 필요 비취를 더함
                    NeedForGameMemory::sharedNeedForGameMemory()->addJadeCount(itemButtonSprite->getNeedJadeCount());
                    
                    ItemInfoSprite *itemInfoSprite    = dynamic_cast<ItemInfoSprite*>(this->getChildByTag(TAG_ITEMSELECTION_INFO_SPRITE));
        ...
        for (unsigned int i=0; i<this->itemChoosedSprites->count(); i++) {
            ...
            if (itemChoosedSprite->boundingBox().containsPoint(pt)) {
                if (this->selectedChoosedSprite == itemChoosedSprite) {
                    // 아이템 선택 버튼들 해제
                    CCLog("selectedChoosedSprite Item Key : %i", this->selectedChoosedSprite->getItemKey());
                    ItemSprite *itemSprite  = dynamic_cast<ItemSprite*>(this->itemButtonSprites->objectAtIndex(this->selectedChoosedSprite->getItemKey()));
                    itemSprite->setIsActive(true);
                    
                    this->selectedChoosedSprite->removeFromParentAndCleanup(true);
                    this->itemChoosedSprites->removeObject(this->selectedChoosedSprite);
                    
                    ItemInfoSprite *itemInfoSprite    = dynamic_cast<ItemInfoSprite*>(this->getChildByTag(TAG_ITEMSELECTION_INFO_SPRITE));
                    itemInfoSprite->reset();
                    
                    // 필요 비취를 뻄
                    NeedForGameMemory::sharedNeedForGameMemory()->minusJadeCount(itemSprite->getNeedJadeCount());
...
int GetNextItemChoosedSpriteKey(CCArray *itemChoosedSprites) {
    ...
    // 현재 있는 선택 아이템 중 비어있는 넘이 있는지 체크. 있으면 그 키 값을 리턴.
    ...
    for (unsigned int i=0; i<ITEM_CHOOSED_TOTAL_COUNT; i++) {
        for (unsigned int j=0; j<itemChoosedSprites->count(); j++) {
            ItemChoosedSprite *itemChoosedSprite    = dynamic_cast<ItemChoosedSprite*>(itemChoosedSprites->objectAtIndex(j));
