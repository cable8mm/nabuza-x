...
void GameLoadingLayer::onEnter() {
    ...
    // 고수방, 중수방, 초보방
    GameEnterLevelSprite *highLevelSprite =
        GameEnterLevelSprite::createWithLevel(kGAMEPLAYERLEVEL_EXPERT);
    highLevelSprite->setPosition(ccp(visibleSize.width / 2, 800. - 274. - 61.));
    this->addChild(highLevelSprite);

    GameEnterLevelSprite *middleLevelSprite =
        GameEnterLevelSprite::createWithLevel(kGAMEPLAYERLEVEL_ADVANCED);
    middleLevelSprite->setPosition(
        ccp(visibleSize.width / 2, 800. - 382. - 61.));
    this->addChild(middleLevelSprite);

    GameEnterLevelSprite *lowLevelSprite =
        GameEnterLevelSprite::createWithLevel(kGAMEPLAYERLEVEL_BEGINNER);
    lowLevelSprite->setPosition(ccp(visibleSize.width / 2, 800. - 483. - 61.));
    this->addChild(lowLevelSprite);
    lowLevelSprite->onAnimating();
    // 고수 중수 초보 얼굴
    GameEnterLevelFaceSprite *highLevelFaceSprite =
        GameEnterLevelFaceSprite::createWithLevel(kGAMEPLAYERLEVEL_EXPERT);
    highLevelFaceSprite->setPosition(ccp(29. + 32., 800. - 274. - 61.));
    this->addChild(highLevelFaceSprite);

    GameEnterLevelFaceSprite *middleLevelFaceSprite =
        GameEnterLevelFaceSprite::createWithLevel(kGAMEPLAYERLEVEL_ADVANCED);
    middleLevelFaceSprite->setPosition(ccp(29. + 32., 800. - 382. - 61.));
    this->addChild(middleLevelFaceSprite);

    GameEnterLevelFaceSprite *lowLevelFaceSprite =
        GameEnterLevelFaceSprite::createWithLevel(kGAMEPLAYERLEVEL_BEGINNER);
...
GameEnterLevelSprite *GameEnterLevelSprite::createWithLevel(
    eGamePlayerLevels level) {
    ...
    if (pobSprite && pobSprite->initWithFile(pobSprite->imageName.c_str(),
                                             CCRect(0., 0., LEVEL_IMAGE_WIDTH,
                                                    LEVEL_IMAGE_HEIGHT))) {
...
void GameEnterLevelSprite::onAnimating() {
    ...
    for (unsigned int i = 0; i < 3; i++) {
        CCSpriteFrame *thisFrame = CCSpriteFrame::createWithTexture(
            thisTexture, CCRect(0, LEVEL_IMAGE_HEIGHT * i, LEVEL_IMAGE_WIDTH,
                                LEVEL_IMAGE_HEIGHT));
        thisFrames->addObject(thisFrame);
    }

    CCAnimation *thisAnimation =
        CCAnimation::createWithSpriteFrames(thisFrames, .2);
    CCSpeed *thisSpeed = CCSpeed::create(
        CCRepeatForever::create(CCAnimate::create(thisAnimation)), 1.);
...
GameEnterLevelFaceSprite *GameEnterLevelFaceSprite::createWithLevel(
    eGamePlayerLevels level) {
    ...
    if (pobSprite &&
        pobSprite->initWithFile(
            pobSprite->imageName.c_str(),
            CCRect(0., 0., LEVEL_FACE_IMAGE_WIDTH, LEVEL_FACE_IMAGE_HEIGHT))) {
        pobSprite->autorelease();
        return pobSprite;
    }
    CC_SAFE_DELETE(pobSprite);
...
void GameEnterLevelFaceSprite::onAnimating() {
    ...
    for (unsigned int i = 0; i < 3; i++) {
        CCSpriteFrame *thisFrame = CCSpriteFrame::createWithTexture(
            thisTexture,
            CCRect(LEVEL_FACE_IMAGE_WIDTH * i, 0, LEVEL_FACE_IMAGE_WIDTH,
                   LEVEL_FACE_IMAGE_HEIGHT));
        thisFrames->addObject(thisFrame);
    }
