...
void BelowStartLayer::onEnter()
{
    ...
    btnStartSprite->setPosition(ccp(visibleSize.width / 2, ATUtil::agy(641.) - 38.));
    this->addChild(btnStartSprite);
    
    CCArray *btnStartFrames  = new CCArray;
    for (int i=0; i < 6; i++) {
        btnStartFrames->addObject(CCSpriteFrame::createWithTexture(btnStartTexture, CCRectMake(0, 76.*i, 290., 76.)));
    }
    CCAnimation* animationBtnStart  = CCAnimation::createWithSpriteFrames(btnStartFrames, .2f);
    CCAnimate*  animateBtnStart = CCAnimate::create(animationBtnStart);
    CCRepeatForever *repeatAnimateBtnStart = CCRepeatForever::create(animateBtnStart);
...
void BelowStartLayer::onStartGame() {
    if (!this->isGameStart) {
        ...
        if (UserStorage::sharedUserStorage()->getJadeCount() < NeedForGameMemory::sharedNeedForGameMemory()->getJadeCount()) {
            AlertLayer::sharedAlertLayer()->show("비취 부족", "비취가 부족합니다.\n비취를 구매해 주세요.");

//            AlertLayer *alertLayer  = AlertLayer::createWithMessage("비취 부족", "비취가 부족합니다.\n비취를 구매해 주세요.");
...
//            CCMessageBox("비취가 부족합니다.\n비취를 구매해 주세요.", "비취 부족");
        ...
        if (UserStorage::sharedUserStorage()->getCardCount() < 1) {
            AlertLayer::sharedAlertLayer()->show("초대장 부족", "초대장이 부족합니다.");
//            AlertLayer *alertLayer  = AlertLayer::createWithMessage("초대장 부족", "초대장이 부족합니다.");
//            this->addChild(alertLayer);
            return;
        }
        ...
        int cardCount   = UserStorage::sharedUserStorage()->getCardCount() > 5? 5 : UserStorage::sharedUserStorage()->getCardCount();
//        float x = cardCount * 44. + 127.+25.+44.;
        float x = visibleSize.width / 2 - 44*3 + 44*cardCount;
        LetterBigSprite *letterBigSprite    = LetterBigSprite::createWithPosition(ccp(x,684.), callfunc_selector(BelowStartLayer::replaceGameScene));
...
bool BelowStartLayer::ccTouchBegan(CCTouch *pTouch, CCEvent *pEvent) {
    ...
    CCSprite *btnStartSprite    = (CCSprite *)this->getChildByTag(TAG_GAME_START_SPRITE);
