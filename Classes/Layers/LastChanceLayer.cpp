...
void LastChanceLayer::onEnter()
{
    ...
    this->playSpeechSprite   = CCSprite::createWithTexture(playSpeechTexture, CCRect(0.,447.,235.,149.));
    playSpeechSprite->setPosition(ccp(203.+117.5, 800.-243.-74.5));    // 78,377
    this->addChild(playSpeechSprite, 9999999+99);
    
    this->runAction(CCSequence::create(
                                       CCDelayTime::create(.5)
                                       ,CCCallFunc::create(this, callfunc_selector(LastChanceLayer::speechStart))
...
void LastChanceLayer::close() {
    CCNotificationCenter::sharedNotificationCenter()->postNotification("NABUZA_LASTCHANCE_NOTIFICATION");
