...
//AlertLayer* AlertLayer::createWithMessage(std::string title, std::string message) {
...
bool AlertLayer::init() {
    ...
    // 닫기 버튼
    ButtonSprite *closeButtonSprite = ButtonSprite::createWithItemKey(kGAMEBUTTON_CONFIRM);
    ButtonSprite *closeButtonSpriteTapped = ButtonSprite::createWithItemKey(kGAMEBUTTON_CONFIRM);
    ...
    CCMenuItemSprite *closeButtonItem    = CCMenuItemSprite::create(closeButtonSprite, closeButtonSpriteTapped, this, menu_selector(AlertLayer::close));
    ...
    titleLabel->setPosition(ccp(visibleSize.width / 2., visibleSize.height / 2. + 50.));
    ...
    messageLabel->setPosition(ccp(visibleSize.width / 2., visibleSize.height / 2.));
