...
void IndicationLayer::onEnter() {
    ...
    CCTexture2D *indicatorTexture =
        CCTextureCache::sharedTextureCache()->addImage("indicator.png");
    CCSprite *indicatorSprite = CCSprite::createWithTexture(
        indicatorTexture, CCRect(0., 0., 125., 125));
    ...
    for (unsigned int i = 0; i < 3; i++) {
        CCSpriteFrame *indicatorFrame = CCSpriteFrame::createWithTexture(
            indicatorTexture, CCRect(125. * i, 0., 125., 125.));
        indicatorFrames->addObject(indicatorFrame);
    }

    CCAnimation *indicatorAnimation =
        CCAnimation::createWithSpriteFrames(indicatorFrames, .1);
    CCSpeed *indicatorSpeed = CCSpeed::create(
        CCRepeatForever::create(CCAnimate::create(indicatorAnimation)), 2.);
