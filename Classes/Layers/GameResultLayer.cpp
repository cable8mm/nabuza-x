...
void GameResultLayer::onEnter()
{
    ...
    // TITLE
    AlertTitleSprite *alertTitleSprite  = AlertTitleSprite::createWithItemKey(kGAMEALERTTITLE_GAMERESULT);
    this->addChild(alertTitleSprite);
    
    // 스코어와 롤 배경
    CCTexture2D *gameResultScoreTexture = CCTextureCache::sharedTextureCache()->addImage("gameresult_score.png");
    CCSprite *gameResultScoreSprite  = CCSprite::createWithTexture(gameResultScoreTexture, CCRect(0., 0., 400., 310.));
    gameResultScoreSprite->setPosition(ccp(visibleSize.width / 2., 800.-264.-155.));
    this->addChild(gameResultScoreSprite);
    
    // 캐릭터
    CCTexture2D *characterTexture = CCTextureCache::sharedTextureCache()->addImage("character.png");
    CCSprite *characterSprite  = CCSprite::createWithTexture(characterTexture, CCRect(0., 0., 200., 250.));
    ...
    LevelInformation::sharedLevelInformation()->setting(ownGoldCount->intValue());
    std::stringstream strLabelLevelBonus;
    strLabelLevelBonus << "레벨 보너스 +";
    std::stringstream ssLevelBonus;
    ssLevelBonus << ((int)(LevelInformation::sharedLevelInformation()->getLevelBonus()*0.01 * (int)this->gameScore));
...
//    strLevelBonus += ATUtil::insert_separator();
    ...
    CCLabelTTF *lbGameResultLevelBonus  = CCLabelTTF::create(strLabelLevelBonus.str().c_str(), "JejuGothic.ttf", 20);
    lbGameResultLevelBonus->setPosition(ccp(visibleSize.width / 2., 800. - 150. - 135.));
    ...
    // 다음 레벨까지
    int needGoldCount   = LevelInformation::sharedLevelInformation()->getNeedGoldCountForNextLevel();
    ...
    CCLabelTTF *lbGameResultNextLevel  = CCLabelTTF::create(strLabelNeedGoldCountForNextLevel.str().c_str(), "JejuGothic.ttf", 18);
    ...
    // 최고 점수
    std::string strLabelUserHighscore   = UserStorage::sharedUserStorage()->getHighscore()->getCString();
    ...
    CCLabelTTF *lbGameResultHighScore  = CCLabelTTF::create(ssLabelUserHighscore.str().c_str(), "JejuGothic.ttf", 20);
    ...
    // 닫기 버튼
    ButtonSprite *closingButtonSprite = ButtonSprite::createWithItemKey(kGAMEBUTTON_CLOSE);
    ButtonSprite *closingButtonSpriteTapped = ButtonSprite::createWithItemKey(kGAMEBUTTON_CLOSE);
    closingButtonSpriteTapped->setScale(1.2);
    closingButtonSpriteTapped->setPosition(ccp(closingButtonSprite->getPositionX()-20., closingButtonSprite->getPositionY()-6.));
//    btnStartSelectedSprite->setPosition(ccp(btnStartSprite->getPositionX()-29., btnStartSprite->getPositionY()-10.));

    
    CCMenuItemSprite *closeButtonItem    = CCMenuItemSprite::create(closingButtonSprite, closingButtonSpriteTapped, this, menu_selector(GameResultLayer::close));
    ...
    // 점수
    CCLabelBMFont *lbGameResultScore  = CCLabelBMFont::create("0", "fontopo30brown.fnt");
    lbGameResultScore->setPosition(ccp(visibleSize.width / 2., 800. - 50. - 190.));
    lbGameResultScore->setScale(2.);
    lbGameResultScore->setAlignment(kCCTextAlignmentCenter);
    this->addChild(lbGameResultScore, LAYER_GAME_RESULT_SCORE, TAG_GAME_RESULT_SCORE);
...
//    AnytaleHTTP::onHttpRequesting(this, callfuncND_selector(LaunchingScene::onHttpRequestCompleted), "/players/regist", "", "HTTP_PLAYERS_REGIST");
    ...
    AnytaleHTTP::onHttpRequesting(this, callfuncND_selector(GameResultLayer::onHttpRequestCompleted), "/players/finished_game2", ss.str().c_str(), "PLAYERS_FINISHED_GAME2");
...
void GameResultLayer::onHttpRequestCompleted(CCHttpClient *sender, CCHttpResponse *response)
{
    Json::Value root;
    ...
    if(strcmp(response->getHttpRequest()->getTag(), "PLAYERS_FINISHED_GAME2") == 0) {
        /*
        ...
        //        bool isRegist   = root["result"].get("is_regist", false).asBool();
        CCLabelBMFont *lbGameResultScore    = dynamic_cast<CCLabelBMFont*>(this->getChildByTag(TAG_GAME_RESULT_SCORE));

        lbGameResultScore->runAction(CCRepeatForever::create(
                                                             CCSequence::create(
                                                                                CCDelayTime::create(0.03),
                                                                                CCCallFunc::create(this, callfunc_selector(GameResultLayer::changeScore)),
                                                                                NULL)
                                                             ));
...
void GameResultLayer::changeScore() {
    ...
    if ((this->gameTickScore > this->gameScore || this->gameTickScore == this->gameScore) && this->kGameResultState == kGAMERESULTSTATE_ADD_COINCOUNT) {
...
void GameResultLayer::setGameResultState(eGameResultStats kGameResultState) {
    ...
    switch (kGameResultState) {
        ...
        case kGAMERESULTSTATE_DISPLAY_GOLD:
        {
            ...
            CCLabelBMFont *lbGameResultGoldBonus  = CCLabelBMFont::create(strGameResultGoldBonus.c_str(), "rst_score.fnt");
            ...
            lbGameResultGoldBonus->runAction(CCEaseElasticOut::create(CCMoveBy::create(.8, ccp(0., -600.)), .3));

            // highscore 갱신되면 테두리 나타남.
            if (true) {
                // 테두리
                CCTexture2D *gameResultTexture = CCTextureCache::sharedTextureCache()->addImage("gameresult_best.png");
                CCSprite *gameResultSprite  = CCSprite::createWithTexture(gameResultTexture, CCRect(0., 0., 480., 551.));
                gameResultSprite->setPosition(ccp(visibleSize.width / 2., 800.-77.-275.5));
                this->addChild(gameResultSprite);

                CCArray *frames  = new CCArray;
                for (unsigned int i=0; i<2; i++) {
                    CCSpriteFrame *frame = CCSpriteFrame::createWithTexture(gameResultTexture, CCRect(480.*i, 0,480.,551.));
                ...
                CCSprite *characterSprite  = (CCSprite *)this->getChildByTag(TAG_CHARACTER_SPRITE); // rect 200.,250.

                CCArray *characterFrames  = new CCArray;
                for (unsigned int i=0; i<4; i++) {
                    CCSpriteFrame *characterFrame = CCSpriteFrame::createWithTexture(characterSprite->getTexture(), CCRect(200.*(i%2), 250.+250.*floorf(i/2),200.,250.));
                    characterFrames->addObject(characterFrame);
                }
                CCAnimation *characterAnimation    = CCAnimation::createWithSpriteFrames(characterFrames, .2);
                CCAnimate *characterAnimate = CCAnimate::create(characterAnimation);
                characterSprite->runAction(CCRepeatForever::create(characterAnimate));
            }
        }
            break;
        case kGAMERESULTSTATE_DONE:
        {
            ...
            CCNotificationCenter::sharedNotificationCenter()->postNotification("NABUZA_END_GAMERESULT_NOTIFICATION");
            this->stopAllActions();
...
void playSoundCoinCount() {
    CocosDenshion::SimpleAudioEngine::sharedEngine()->playEffect("CU001.mp3");  // 3초임
}
