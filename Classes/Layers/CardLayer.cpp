...
void CardLayer::onEnter()
{
    ...
    CCNotificationCenter::sharedNotificationCenter()->addObserver(this, callfuncO_selector(CardLayer::onChangedCardCount), "OWN_CARD_COUNT_CHANGED_NOTIFICATION", NULL);

    CCSize visibleSize = CCDirector::sharedDirector()->getVisibleSize();
    
    CCSprite *letterBarSprite   = CCSprite::create("letterbar.png");
    letterBarSprite->setPosition(ccp(visibleSize.width / 2, ATUtil::agy(78.) - 34.));
    this->addChild(letterBarSprite);
    
    CCTexture2D *leftLetterTexture = CCTextureCache::sharedTextureCache()->addImage("letter_c.png");
    CCTexture2D *rightLetterTexture = CCTextureCache::sharedTextureCache()->addImage("letter_c_r.png");
    ...
    CCSprite *rightLetterSprite  = CCSprite::createWithTexture(leftLetterTexture);
    ...
    for (unsigned int i = 0; i < 2; i++) {
        ...
        for (unsigned int ii=0; ii < 5; ii++) {
            if (i == 0) {
                btnletterBarFrames->addObject(CCSpriteFrame::createWithTexture(leftLetterTexture, CCRectMake(0, 60.*ii, 60., 60.)));
            } else {
                btnletterBarFrames->addObject(CCSpriteFrame::createWithTexture(rightLetterTexture, CCRectMake(0, 60.*ii, 60., 60.)));
            }
        }
        CCAnimation* animationTableObject  = CCAnimation::createWithSpriteFrames(btnletterBarFrames, .2f);
    ...
    // 카드 갯수
    CCLabelTTF *lbCircleCardCount    = CCLabelTTF::create("", DEFAULT_FONT_NAME, 16);
    ...
    // 플러스 버튼 밑의 MAX 표시(카드가 5개 이상이면 MAX가 뜨고, 4개까지 일 경우 시간이 뜬다.)
    ...
    unsigned int diffSeconds    = UserStorage::sharedUserStorage()->getDiffSeconds();
    int displayCardCount   = ownCardCount > MAX_CARD_COUNT ? MAX_CARD_COUNT : ownCardCount;
...
void CardLayer::setDisplayCardCount(int cardCount) {
    int visibleCardCount    = cardCount > MAX_CARD_COUNT? MAX_CARD_COUNT : cardCount;
    CCLog("cardCount : %d, visibleCardCount : %d", cardCount, visibleCardCount);
    // 카드
    for (unsigned int i=0; i < MAX_CARD_COUNT; i++) {
        CCSprite *cardSprite    = (CCSprite*)this->getChildByTag(TAG_CARD_SPRITE_INITIAL+i);
    ...
    // 카드 갯수
    CCSprite *bgCardCountSprite = (CCSprite*)this->getChildByTag(TAG_CARD_COUNT_BG_SPRITE);
    CCLabelTTF *lbCircleCardCount    = (CCLabelTTF*)this->getChildByTag(TAG_CARD_COUNT_LABEL);
...
void CardLayer::tick() {
    CCLabelTTF *lbRemainSeconds = (CCLabelTTF *)this->getChildByTag(TAG_MAX_LABEL);
    this->countDownClock--;
...
void CardLayer::ccTouchEnded(CCTouch *pTouch, CCEvent *pEvent) {
    ...
    if (plusMaxSprite->boundingBox().containsPoint(pt)) {
        ...
        SaleInvitationLayer *saleInvitationLayer    = SaleInvitationLayer::create();
