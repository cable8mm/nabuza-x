...
{
    CCLog("NoticeLayer width : %f, height: %f", this->getContentSize().width, this->getContentSize().height);
    this->setTouchEnabled(true);
    AnytaleHTTP::onHttpRequesting(this, callfuncND_selector(NoticeLayer::onHttpRequestCompleted), "/notices/last2", "", "NOTICES_LAST2");
    
    CCLayerColor::onEnter();
}

void NoticeLayer::onHttpRequestCompleted(CCHttpClient *sender, CCHttpResponse *response) {
    Json::Value root;
    Json::Reader reader;
...
//            
//            CCSprite *alertView = CCSprite::create("what_alert.png");
//            alertView->setAnchorPoint(ccp(.5, .5));
//            alertView->setPosition(ccp(visibleSize.width/2, visibleSize.height/2)); // 콘텐트의 절대 좌표가 들어감.
//            scrollContainerLayer->addChild(alertView);
//            scrollContainerLayer->setPosition(CCPointZero);
//            scrollContainerLayer->setContentSize(CCSizeMake(visibleSize.width, visibleSize.height * .7));
//            
//            CCScrollView *scrollView = CCScrollView::create(CCSizeMake(480., 400.), scrollContainerLayer);
...
//            this->addChild( scrollView );
            ...
            CCMenuItemSprite *closeButtonItem    = CCMenuItemSprite::create(closeButtonSprite, closeButtonSpriteTapped, this, menu_selector(NoticeLayer::viewContents));
...
void NoticeLayer::viewContents() {
    ...
    NoticeModel *notice = (NoticeModel *)this->notices->objectAtIndex(this->currentNoticeIdx++);
    CCLabelTTF *lbNabuzaInfomation    = CCLabelTTF::create(notice->contents.c_str(), "JejuGothic", 20.);
    lbNabuzaInfomation->setHorizontalAlignment(kCCTextAlignmentCenter);
    lbNabuzaInfomation->setPosition(ccp(visibleSize.width / 2, visibleSize.height / 2));
    lbNabuzaInfomation->setAnchorPoint(ccp(.5, .5));
    lbNabuzaInfomation->setColor(ccBLACK);
    
//    CCSprite *alertView = (CCSprite *)this->getChildByTag(TAG_ALERTVIEW_SPRITE);
