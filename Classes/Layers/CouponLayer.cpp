...
void CouponLayer::onEnter() {
    ...
    CCMenuItemImage *btnCloseItem =
        CCMenuItemImage::create("exit_btn.png", "exit_btn.png", this,
                                callfuncO_selector(CouponLayer::close));
    ...
    for (unsigned int i = 0; i < 4; i++) {
        ...
        displayNumber->setPosition(
            ccp(NUMBER_LABEL_LEFT + i * 95., KEYPAD_POSITION_X + 200.));
    ...
    CCMenuItemSprite *couponButtonItem = CCMenuItemSprite::create(
        ...
        menu_selector(CouponLayer::registerCoupon));
...
void CouponLayer::registerCoupon() {
    if (this->couponNumber.str().length() != 16) {
        ...
        //        AlertLayer *alertLayer  = AlertLayer::createWithMessage("쿠폰
        //        적용 실패", "쿠폰 번호가 유효하지 않습니다.\n다시
        //        확인해주세요."); this->addChild(alertLayer);
        return;
    }
    ...
    AnytaleHTTP::onHttpRequesting(
        this, callfuncND_selector(CouponLayer::onHttpRequestCompleted),
        "/coupons/using2", ss.str().c_str(), "COUPONS_USING2");
...
void CouponLayer::onHttpRequestCompleted(CCHttpClient *sender,
    ...
    if (strcmp(response->getHttpRequest()->getTag(), "COUPONS_USING2") == 0) {
        ...
        if (code != 0) {
            ...
            //            AlertLayer::createWithMessage("쿠폰 적용 실패", "쿠폰
            //            번호가 유효하지 않습니다.\n다시 확인해주세요.");
        ...
        int jadeCount =
            root["result"]["CouponIssue"].get("jade_count", 0).asInt();
        std::string sponsorName =
            root["result"]["CouponSponsor"].get("name", "").asString();
        ...
        //        AlertLayer *alertLayer  = AlertLayer::createWithMessage("쿠폰
        //        적용", ss.str()); this->addChild(alertLayer);
        return;
    }
...
void CouponLayer::addCouponNumber(int v) {
    ...
    if (v == 11 && this->couponNumber.str().length() > 0) {  // 한자 지우기
        std::string tmp = this->couponNumber.str().erase(
            this->couponNumber.str().length() - 1);
...
void CouponLayer::drawCouponNumber() {
    for (unsigned int i = 0; i < 4; i++) {
        CCLabelTTF *displayNumber =
            (CCLabelTTF *)this->getChildByTag(TAG_DISPLAY_NUMBER_INITIAL + i);
    ...
    int groupCount = (int)floorf((this->couponNumber.str().length() - 1) /
                                 4);  // 칸 순번은 0,1,2,3
