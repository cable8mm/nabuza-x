...
void MessageBoxLayer::onEnter()
{
    ...
    // TITLE
    AlertTitleSprite *alertTitleSprite  = AlertTitleSprite::createWithItemKey(kGAMEALERTTITLE_MESSAGE_BOX);
    this->addChild(alertTitleSprite);
    
    AnytaleHTTP::onHttpRequesting(this, callfuncND_selector(MessageBoxLayer::onHttpRequestCompleted), "/messages/get2", "", "MESSAGES_GET2");
    
    CCLayerColor::onEnter();
}

void MessageBoxLayer::onHttpRequestCompleted(CCHttpClient *sender, CCHttpResponse *response) {
    Json::Value root;
    Json::Reader reader;
    ...
    if(strcmp(response->getHttpRequest()->getTag(), "MESSAGES_GET2") == 0) {
        /*
         {"code":0,"message":"OK","result":[{"Message":{"id":"125","sent_player_id":"57","nickname":"\uae40\uc724\uc804","own_gold":"373503","received_player_id":"93","type":"0","is_used":"0","modified":"2013-01-18 01:28:25","created":"2013-01-18 01:28:25"}}]}
         */
        ...
        CCMenuItemSprite *closeButtonItem    = CCMenuItemSprite::create(closeButtonSprite, closeButtonSpriteTapped, this, menu_selector(MessageBoxLayer::close));
        
        CCMenu *buttonMenu;
        if (rankings.size() != 0) {
            ButtonSprite *receiveAllButtonSprite = ButtonSprite::createWithItemKey(kGAMEBUTTON_RECEIVE_ALL);
            ButtonSprite *receiveAllButtonSpriteTapped = ButtonSprite::createWithItemKey(kGAMEBUTTON_RECEIVE_ALL);
            receiveAllButtonSpriteTapped->setScale(1.2);
            receiveAllButtonSpriteTapped->setPosition(ccp(receiveAllButtonSprite->getPositionX()-20., receiveAllButtonSprite->getPositionY()-6.));
            CCMenuItemSprite *receiveAllButtonItem    = CCMenuItemSprite::create(receiveAllButtonSprite, receiveAllButtonSpriteTapped, this, menu_selector(MessageBoxLayer::close));
            buttonMenu    = CCMenu::create(receiveAllButtonItem, closeButtonItem, NULL);
        } else {
        ...
        // 하단 메시지
        CCLabelTTF *lbNabuzaInfomation    = CCLabelTTF::create("주의 : 메시지함은 최대 77건까지만 보관 됩니다.", "JejuGothic", 20);
        ...
        if (rankings.size() == 0) {
            ...
            CCTexture2D *characterTexture = CCTextureCache::sharedTextureCache()->addImage("character_2.png");
            CCArray *characterFrames  = new CCArray;
            for (unsigned int i=0; i<2; i++) {
                CCSpriteFrame *characterFrame = CCSpriteFrame::createWithTexture(characterTexture, CCRect(180.*i,0,180.,250.));
                characterFrames->addObject(characterFrame);
            }
            CCAnimation *characterAnimation    = CCAnimation::createWithSpriteFrames(characterFrames, .1);
            
            CCSprite *character  = CCSprite::createWithTexture(characterTexture, CCRect(0., 0., 180., 250.));
            character->setPosition(ccp(visibleSize.width / 2, 800.-400.));
            this->addChild(character);
            
            CCLabelTTF *lbNabuzaInfomation    = CCLabelTTF::create("신규 메시지가 없습니다.", "JejuGothic", 20);
            lbNabuzaInfomation->setHorizontalAlignment(kCCTextAlignmentCenter);
            lbNabuzaInfomation->setPosition(ccp(visibleSize.width / 2, 800.-550.));
            lbNabuzaInfomation->setAnchorPoint(ccp(.5, .5));
            lbNabuzaInfomation->setColor(ccBLACK);
            this->addChild(lbNabuzaInfomation);
            
            character->runAction(CCSpeed::create(CCRepeatForever::create(CCAnimate::create(characterAnimation)), .5));
        } else {
            for (int index=0; index < rankings.size(); index++) {
                MessageModel *message = new MessageModel;
                message->idx      = rankings[index]["Message"]["id"].asInt();
                message->nickname = rankings[index]["Message"]["nickname"].asString();
                message->ownGoldCount = rankings[index]["Message"]["own_gold"].asInt();
                message->type    = rankings[index]["Message"]["type"].asInt();
                message->isUsed = rankings[index]["Message"]["is_used"].asBool();
                this->messages->addObject(message);
            }
            
            CCTableView *tableView = CCTableView::create(this, CCSizeMake(400., 400.));
    ...
    if(strcmp(response->getHttpRequest()->getTag(), "MESSAGES_USE_INVITATION2") == 0) {
        CCTableView *tableView  = (CCTableView *)this->getChildByTag(TAG_MESSAGES_TABLEVIEW);
        CCTableViewCell *cell   = tableView->cellAtIndex(this->useInvitationCellIdx);
...
CCTableViewCell* MessageBoxLayer::tableCellAtIndex(CCTableView *table, unsigned int idx)
{
    MessageModel *message = (MessageModel*)(this->messages->objectAtIndex(idx));
    ...
    if (!cell) {
        ...
        // 레벨아이콘
        LevelIconSprite *levelIconSprite    = LevelIconSprite::createWithGoldCount(message->ownGoldCount);
        ...
        ssNickname << message->nickname.c_str() << " 님이\n초대장을 보냈습니다.";
        CCLabelTTF *lbNickName = CCLabelTTF::create(ssNickname.str().c_str(), DEFAULT_FONT_NAME, 18.0);
...
void MessageBoxLayer::tableCellTouched(CCTableView* table, CCTableViewCell* cell)
{
    CCLOG("cell touched at index: %i", cell->getIdx());
    MessageModel *message = (MessageModel*)(this->messages->objectAtIndex(cell->getIdx()));
    
    if (this->tappedPoint.x > 370 && this->tappedPoint.x < 440) {
            std::stringstream ss;
            ss << "&id=" << message->idx;
            AnytaleHTTP::onHttpRequesting(this, callfuncND_selector(MessageBoxLayer::onHttpRequestCompleted), "/messages/use_invitation2", ss.str().c_str(), "MESSAGES_USE_INVITATION2");
