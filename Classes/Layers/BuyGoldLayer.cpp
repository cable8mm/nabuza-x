...
void BuyGoldLayer::onEnter()
{
    ...
    // TITLE
    AlertTitleSprite *alertTitleSprite  = AlertTitleSprite::createWithItemKey(kGAMEALERTTITLE_BUY_GOLD);
    ...
    CCMenuItemSprite *closeButtonItem    = CCMenuItemSprite::create(closeButtonSprite, closeButtonSpriteTapped, this, menu_selector(BuyGoldLayer::close));
    ...
    for (unsigned int i=0; i < GOLDBUTTON_COUNT; i++) {
        BuyGoldButtonSprite *goldButtonSprite = BuyGoldButtonSprite::createWithItemKey(i);
        BuyGoldButtonSprite *goldButtonSpriteTapped = BuyGoldButtonSprite::createWithItemKeyTapped(i);
        CCMenuItemSprite *goldButtonItem    = CCMenuItemSprite::create(goldButtonSprite, goldButtonSpriteTapped, this, menu_selector(BuyGoldLayer::onBuyGoldButtonTapped));
...
void BuyGoldLayer::onBuyGoldButtonTapped(CCObject *object) {
    ...
    CCAssert(needJadeCountKey >= 0 && needJadeCountKey < GOLDBUTTON_COUNT, "not validate tag value");
    
    int needJadeCounts[]   = {10, 50, 100, 300, 500};
    int buyedGoldCounts[]    = {2000, 11000, 24000, 80000, 16000};
    
    if (UserStorage::sharedUserStorage()->getJadeCount() < needJadeCounts[needJadeCountKey]) {
...
void BuyGoldLayer::onHttpRequestCompleted(CCHttpClient *sender, CCHttpResponse *response) { 
    Json::Value root;
    ...
    // 게임 시작을 서버에 알림
    if(strcmp(response->getHttpRequest()->getTag(), "SPENT_JADES_BUY_GOLD2") == 0) {
