...
const char *USER_DEFAULT_SETTING_STORAGE_EFFECT_KEY =
    "SETTING_STORAGE_EFFECT_KEY";
const char *USER_DEFAULT_SETTING_STORAGE_VIBRATION_KEY =
    "SETTING_STORAGE_VIBRATION_KEY";
const char *USER_DEFAULT_SETTING_STORAGE_EVENTALARM_KEY =
    "SETTING_STORAGE_EVENTALARM_KEY";
...
void SettingLayer::onEnter() {
    ...
    // 설정 읽어 옴.
    bool isBgm =
        userDefault->getBoolForKey(USER_DEFAULT_SETTING_STORAGE_BGM_KEY, true);
    bool isEffect = userDefault->getBoolForKey(
        USER_DEFAULT_SETTING_STORAGE_EFFECT_KEY, true);
    bool isVibration = userDefault->getBoolForKey(
        USER_DEFAULT_SETTING_STORAGE_VIBRATION_KEY, true);
    bool isEventalarm = userDefault->getBoolForKey(
        USER_DEFAULT_SETTING_STORAGE_EVENTALARM_KEY, true);
    ...
    CCLabelTTF *lbNabuzaInfomation =
        CCLabelTTF::create(nabuzaInfomation.str().c_str(), "JejuGothic", 20);
    ...
    // 도움말 / 페이스북 로그인
    ButtonSprite *helpButtonSprite =
        ButtonSprite::createWithItemKey(kGAMEBUTTON_HELP);
    ButtonSprite *helpButtonSpriteTapped =
        ButtonSprite::createWithItemKey(kGAMEBUTTON_HELP);
    helpButtonSpriteTapped->setScale(1.2);
    helpButtonSpriteTapped->setPosition(
        ccp(helpButtonSprite->getPositionX() - 20.,
            helpButtonSprite->getPositionY() - 6.));

    ButtonSprite *facebookLoginButtonSprite =
        ButtonSprite::createWithItemKey(kGAMEBUTTON_FACEBOOK_LOGIN);
    ButtonSprite *facebookLoginButtonSpriteTapped =
        ButtonSprite::createWithItemKey(kGAMEBUTTON_FACEBOOK_LOGIN);
    facebookLoginButtonSpriteTapped->setScale(1.2);
    facebookLoginButtonSpriteTapped->setPosition(
        ccp(facebookLoginButtonSprite->getPositionX() - 20.,
            facebookLoginButtonSprite->getPositionY() - 6.));

    CCMenuItemSprite *helpButtonItem =
        CCMenuItemSprite::create(helpButtonSprite, helpButtonSpriteTapped, this,
                                 menu_selector(SettingLayer::openHelpView));
    CCMenuItemSprite *facebookButtonItem = CCMenuItemSprite::create(
        facebookLoginButtonSprite, facebookLoginButtonSpriteTapped, this,
        menu_selector(SettingLayer::close));
    ...
    CCMenuItemImage *btnCloseItem =
        CCMenuItemImage::create("exit_btn.png", "exit_btn.png", this,
                                callfuncO_selector(SettingLayer::close));
...
void SettingLayer::openHelpView() {
    WebViewLayer *webViewLayer =
        WebViewLayer::createWithUrl("http://gs.anytale.com/nabuza_help_1.htm");
...
void SettingLayer::ccTouchEnded(CCTouch *touch, CCEvent *event) {
    ...
    if (this->eventalarmSwitch->boundingBox().containsPoint(pt)) {
        ...
        AnytaleHTTP::onHttpRequesting(
            this, callfuncND_selector(SettingLayer::onHttpRequestCompleted),
            "/players/set_notifications", ss.str().c_str(),
            "PLAYERS_SET_NOTIFICATIONS");
        return;
    }
}
...
void SettingLayer::onHttpRequestCompleted(CCHttpClient *sender,
